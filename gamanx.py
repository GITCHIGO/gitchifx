...
class CHoCHDetector:
    ...
    def detect_signal(self, df, pair):
        ...
        # Detect swings tot bar n-swing_length (kunnen niet recent confirmen)
        swings = detect_swing_points(df, length=self.swing_length)
        if len(swings) < 4:
            return None

        # Bepaal heersende trend state
        trend, last_high, last_low = self._classify_trend(swings)
        if trend == "NEUTRAL":
            return None

        # Check laatste bar for CHoCH break (close-based, niet wick)
        ...
        if trend == "BEAR":
            # Bullish CHoCH: close above last Lower High
            if latest_close > last_high["price"]:
                signal_dir = "LONG"
                broken_level = last_high["price"]
                # SL voorbij laatste LL (most recent low)
                sl_level = last_low["price"]
        elif trend == "BULL":
            # Bearish CHoCH: close below last Higher Low
            if latest_close < last_low["price"]:
                signal_dir = "SHORT"
                broken_level = last_low["price"]
                # SL voorbij laatste HH
        ...
        if self.use_pullback:
            # Confirmation variant: entry op broken_level (retest), niet op CHoCH close
            # In live mode betekent dit: signaal staat klaar, entry zodra prijs broken_level raakt
            # Voor MVP: gebruiken we directe entry op broken_level (klassiek SMC retest)
            entry_price = broken_level
        else:
            # Aggressive: entry op CHoCH close
...
class BOSDetector:
    ...
    def detect_signal(self, df, pair):
        ...
        BOS = trend continuation break. In BULL trend, prijs maakt nieuwe HH die
        de vorige HH breekt → continuation signal LONG. In BEAR trend, nieuwe LL
        breekt vorige LL → SHORT.
        ...
        if trend == "BULL":
            # Bullish BOS: close above last Higher High → continuation
            if latest_close > last_high["price"]:
                signal_dir = "LONG"
                broken_level = last_high["price"]
                # SL voorbij laatste HL (most recent low van pullback)
                sl_level = last_low["price"]
        elif trend == "BEAR":
            # Bearish BOS: close below last Lower Low → continuation
            if latest_close < last_low["price"]:
                signal_dir = "SHORT"
                broken_level = last_low["price"]
                # SL voorbij laatste LH
                sl_level = last_high["price"]

        if signal_dir is None:
            return None

        # Entry op gebroken level (klassieke retest)
        # In live: signaal staat klaar, executie wanneer prijs broken_level raakt
        # In MVP: direct entry op broken_level
        entry_price = broken_level
        pip_size = PIP.get(pair, 0.0001)
...
def backtest_silver_bullet(pair, tf, start_date, end_date, capital=10000,
    ...
    return {"trades": trades, "stats": _compute_backtest_stats(trades, capital)}
...
def backtest_choch(pair, tf, start_date, end_date, capital=10000,
    ...
    if not trades:
        return {"trades": [], "stats": {"total": 0}, "msg": "No CHoCH setups found in this period"}
    return {"trades": trades, "stats": _compute_backtest_stats(trades, capital)}
...
def backtest_bos(pair, tf, start_date, end_date, capital=10000,
    ...
    if not trades:
        return {"trades": [], "stats": {"total": 0}, "msg": "No BOS setups found in this period"}
    return {"trades": trades, "stats": _compute_backtest_stats(trades, capital)}
...
def backtest_asian_breakout(pair, tf, start_date, end_date, capital=10000,
    ...
    # Itereer per dag binnen de date range
    start_dt = pd.to_datetime(start_date).date()
    end_dt = pd.to_datetime(end_date).date()
    current = start_dt + timedelta(days=1)  # eerste valid dag (need prior 23:00)

    while current <= end_dt:
        # Skip weekend (no Asian session)
        if current.weekday() >= 5:
            current += timedelta(days=1)
            continue

        # Find Asian range voor deze dag
        ...
        # Breakout window end_idx: target 12:00 Brussels
        ...
        # Simulate forward from retest
        entry_idx = signal["retest_idx"]
        # Max 50 bars forward (or end of day) for exit
    ...
    if not trades:
        return {"trades": [], "stats": {"total": 0}, "msg": "No Asian Breakout setups found in this period"}
    return {"trades": trades, "stats": _compute_backtest_stats(trades, capital)}
...
def api_backtest():
    ...
    if strategy not in ("SB", "CH", "BOS", "AB"):
  ...
  <div class="form-row">
    <div class="form-group">
      ...
      <select id="btStrat">
        ...
        <option value="CH">CHoCH Reversal</option>
        <option value="BOS">BOS Continuation</option>
        <option value="AB">Asian Range Breakout</option>
      </select>
    </div>
...
async function runBacktest(){
  ...
  const body = {
    ...
    killzones,
    // AB params — pulled from right panel
    min_range_pips: parseInt(document.getElementById("abMinRange").value) || null,
    max_range_pips: parseInt(document.getElementById("abMaxRange").value) || null,
    tp_range_mult: parseFloat(document.getElementById("abTpMult").value)||1.5,
    use_rr: document.getElementById("abUseRR").value === "true",
