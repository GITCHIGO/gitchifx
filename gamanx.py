  ...
  <div class="card collapse-section">
    ...
    <div class="collapse-body">
      <div class="form-row">
        ...
        <div class="form-group"><label>Timeframe</label><select id="cfgTF"><option value="15M" selected>15M</option><option value="1H">1H</option><option value="4H">4H</option></select></div>
      </div>
      <div class="form-row">
...
function renderOpenTrades(){
  ...
  el.innerHTML = `<div class="trades-list">${trades.map(t => `
    <div class="trade-row">
      <div class="row1">
        <span><b>${t.id}</b> <span class="dir-${t.direction}">${t.direction}</span> ${t.pair}</span>
      ...
      <div class="row2">
        ...
        <span>Live ${(t.live_price||0).toFixed(5)}</span>
        <button class="btn danger" style="width:auto;padding:3px 10px;font-size:9px" onclick="closeTrade('${t.id}')">✕ Close</button>
      </div>
    </div>
  `).join("")}</div>`;
}
  ...
  try{
    if(!isCombined){
      ...
      renderBacktestResult({
        mode:"combined-tf",
        trades:allTrades,
        stats:{total:allTrades.length, wr:allTrades.length?Math.round(1000*wins.length/allTrades.length)/10:0, pnl:Math.round(totalPnl*100)/100, pf:0, max_dd:0},
        per_tf:{"15M":r15.stats, "1H":r1h.stats},
        diagnostics:{tf_15M:r15.diagnostics || r15.diagnostics_per_strategy, tf_1H:r1h.diagnostics || r1h.diagnostics_per_strategy},
      });
    }
...
function renderBacktestResult(d){
  ...
  if(trades.length){
    html += `<div class="card-title" style="margin:14px 0 6px">Trades (last 20)</div>
      <div class="trades-list">${trades.slice(-20).reverse().map(t => `
        <div class="trade-row">
          <div class="row1">
