...
PIP     = {"EURUSD": 0.0001, "GBPUSD": 0.0001, "XAUUSD": 0.10, "SPX500": 1.0, "US30": 1.0, "BTCUSD": 1.0, "ETHUSD": 0.10}
PIP_EUR = {"EURUSD": 0.10, "GBPUSD": 0.10, "XAUUSD": 0.92, "SPX500": 0.92, "US30": 0.92, "BTCUSD": 0.92, "ETHUSD": 0.09}
# Instrument category per pair — determines which lot setting applies
PAIR_CATEGORY = {
...
def lot_for_pair(config, pair):
    """Return the configured lot for a pair based on its instrument category."""
    cat = PAIR_CATEGORY.get(pair, "forex")
    if cat == "forex":  return config.get("lot_eur", 10)
    if cat == "metal":  return config.get("lot_xau", 1)
    if cat == "index":  return config.get("lot_index", 1)
    if cat == "crypto": return config.get("lot_crypto", 1)
    return config.get("lot_eur", 10)

# TradingView symbols
TV_SYMBOLS = {"EURUSD": "FX:EURUSD", "GBPUSD": "FX:GBPUSD", "XAUUSD": "OANDA:XAUUSD", "SPX500": "OANDA:SPX500USD", "US30": "OANDA:US30USD", "BTCUSD": "BITSTAMP:BTCUSD", "ETHUSD": "BITSTAMP:ETHUSD"}
YF_SYMBOLS = {"EURUSD": "EURUSD=X", "GBPUSD": "GBPUSD=X", "XAUUSD": "GC=F", "SPX500": "^GSPC", "US30": "^DJI", "BTCUSD": "BTC-USD", "ETHUSD": "ETH-USD"}
...
def fetch_price(pair):
    """Live price fetch met TradingView primary, yfinance fallback."""
    # Try TV first
    try:
        ...
        log("WARN", f"fetch_price TV failed for {pair}: {e}")
    # Fallback to yfinance
    try:
        df = fetch_candles_yf(pair, "15M")
        if df is not None and len(df) > 0:
            price = float(df["close"].iloc[-1])
            if price > 0:
                return price
    except Exception as e:
        log("WARN", f"fetch_price YF failed for {pair}: {e}")
    return None
...
class GamanXEngine:
    ...
    def __init__(self):
        ...
        # Signal cooldown tracking: key="SID|PAIR|TF|ENTRY_BUCKET", value=timestamp
        # Voorkomt dezelfde setup opnieuw triggeren na close/delete
        self.signal_history = {}
        self._load_state()
    ...
    def _default_config(self):
        return {
            ...
            "capital": 10000,
            "lot_eur": 10,                   # forex micro lots (EURUSD, GBPUSD)
            "lot_xau": 1,                    # metal micro lots (XAUUSD)
            "lot_index": 1,                  # index micro lots (SPX500, US30)
            "lot_crypto": 1,                 # crypto micro lots (BTCUSD, ETHUSD)
            "rr": 2.0,
            "discord_webhook": "",
            "strategies": {
    ...
    def _load_state(self):
        ...
        try:
            ...
            if not self.config.get("tfs") or not isinstance(self.config.get("tfs"), list):
                self.config["tfs"] = ["15M"]
            log("INFO", f"State loaded: {len(self.open_trades)} open, {len(self.closed_trades)} closed, {len(self.signal_history)} signals tracked")
        except Exception as e:
    ...
    def start(self):
        ...
        self._notify_engine_event("STARTED")
        return True
    ...
    def stop(self):
        ...
        self._save_state()
        if was_running:
            self._notify_engine_event("STOPPED")
    ...
    def _notify_engine_event(self, event):
        """Send Discord notification on engine state change."""
        url = self.config.get("discord_webhook", "")
        if not url: return
        try:
            icon = {"STARTED": "🟢", "STOPPED": "🔴", "PAUSED": "🟡", "RESUMED": "🟢"}.get(event, "ℹ️")
            tfs = ", ".join(self.config.get("tfs", []))
            strats = [sid for sid, sc in self.config.get("strategies", {}).items() if sc.get("enabled")]
            msg = (
                f"{icon} **GAMAN-X engine {event}**\n"
                f"Time: `{fmt_brussels()}`\n"
                f"Pair: `{self.config.get('pair', '?')}` · TFs: `{tfs or '?'}`\n"
                f"Strategies: `{', '.join(strats) if strats else 'none'}`\n"
                f"Open trades: `{len(self.open_trades)}`"
            )
            requests.post(url, json={"content": msg, "username": "GAMAN-X"}, timeout=4)
        except Exception as e:
            log("WARN", f"Discord notify failed: {e}")
    ...
    def _do_scan(self):
        ...
        if cfg_pairs and isinstance(cfg_pairs, list) and len(cfg_pairs) > 0:
            ...
            # Legacy: "BOTH" = EURUSD+XAUUSD (backward compat with old configs)
            pairs = ["EURUSD","XAUUSD"] if self.config.get("pair") == "BOTH" else [self.config.get("pair", "EURUSD")]
                ...
                for pair in pairs:
                    try:
                        ...
                        if signal:
                            ...
                            # 3-laagse dedup:
                            #  1. Open similar trade (zelfde strategy+pair+direction op ±3 pip)
                            #  2. Signal history cooldown (zelfde setup binnen 4h niet opnieuw, ook na delete)
                            #  3. Recent trade check (legacy fallback)
                            if self._has_open_similar_trade(sid, pair, direction, entry_px, pair_eps_pips=3):
                                log("INFO", f"Skip {sid} {pair} {tf}: similar trade already open at {entry_px:.5f}")
                                continue
                            if self._signal_in_cooldown(sid, pair, tf, direction, entry_px, hours=4):
                                log("INFO", f"Skip {sid} {pair} {tf}: signal cooldown active for {entry_px:.5f}")
                                continue
                            ...
                            # Register signal in cooldown history
                            self._record_signal(sid, pair, tf, direction, entry_px)
                    except Exception as e:
                        log("WARN", f"Scan {sid} {pair} {tf}: {e}")

    def _has_recent_trade(self, sid, pair, tf=None, minutes=30):
        """Check if we've opened a similar trade recently (strategy+pair+tf)."""
    ...
    def _has_open_similar_trade(self, sid, pair, direction, entry, pair_eps_pips=3):
        """Check if er al een OPEN trade is met zelfde strategy+pair+direction
        en entry binnen `pair_eps_pips` pip range.
        """
        pip_size = PIP.get(pair, 0.0001)
        eps = pair_eps_pips * pip_size
        for t in self.open_trades:
            if t.get("strategy") != sid or t.get("pair") != pair:
                continue
            if t.get("direction") != direction:
                continue
            if abs(t.get("entry", 0) - entry) <= eps:
                return True
        return False

    def _signal_key(self, sid, pair, tf, direction, entry):
        """Build a stable key for signal_history.
        Entry wordt gebucket op pip-niveau (afgerond naar 3 pip) zodat kleine
        prijsverschillen niet als 'nieuwe signal' tellen.
        """
        pip_size = PIP.get(pair, 0.0001)
        bucket = round(entry / (pip_size * 3)) * 3  # 3-pip buckets
        return f"{sid}|{pair}|{tf}|{direction}|{bucket}"

    def _signal_in_cooldown(self, sid, pair, tf, direction, entry, hours=4):
        """Check of dit signal recent al getriggerd is — cooldown gehandhaafd ook
        nadat trade geclosed/gedelete is. Voorkomt detector loop op zelfde swing."""
        key = self._signal_key(sid, pair, tf, direction, entry)
        last_ts = self.signal_history.get(key, 0)
        if not last_ts: return False
        cutoff = (now_brussels() - timedelta(hours=hours)).timestamp()
        return last_ts >= cutoff

    def _record_signal(self, sid, pair, tf, direction, entry):
        """Register signal in cooldown history."""
        key = self._signal_key(sid, pair, tf, direction, entry)
        self.signal_history[key] = int(now_brussels().timestamp())
        # Cleanup oude entries (>48h) om history compact te houden
        cutoff = (now_brussels() - timedelta(hours=48)).timestamp()
        self.signal_history = {k: v for k, v in self.signal_history.items() if v >= cutoff}
    ...
    def _open_trade(self, signal):
        ...
        lot = lot_for_pair(self.config, signal["pair"])
        ...
        trade = {
            ...
            "pnl_eur":     0.0,
            "live_price":  signal["entry"],  # initial = entry, monitor zal updaten
            "status":      "OPEN",
        }
    ...
    def _compute_overall_stats(self):
        """Aggregate stats over all strategies — total + daily + breakdowns."""
        all_closed = self.closed_trades
        wins = [t for t in all_closed if t.get("pnl_eur", 0) > 0]
        losses = [t for t in all_closed if t.get("pnl_eur", 0) <= 0]
        total_pnl = sum(t.get("pnl_eur", 0) for t in all_closed)
        sum_wins = sum(t.get("pnl_eur", 0) for t in wins)
        sum_losses = abs(sum(t.get("pnl_eur", 0) for t in losses))
        # Profit factor
        pf = round(sum_wins / sum_losses, 2) if sum_losses > 0 else (999.0 if sum_wins > 0 else 0)
        # Max drawdown via equity curve
        equity = 0.0
        peak = 0.0
        max_dd = 0.0
        for t in all_closed:
            equity += t.get("pnl_eur", 0)
            if equity > peak: peak = equity
            dd = equity - peak
            if dd < max_dd: max_dd = dd
        # Daily P&L (vandaag in Brussels)
        today_str = now_brussels().strftime("%Y-%m-%d")
        today_trades = [t for t in all_closed if str(t.get("closed_at", "")).startswith(today_str)]
        today_pnl = sum(t.get("pnl_eur", 0) for t in today_trades)
        today_wins = len([t for t in today_trades if t.get("pnl_eur", 0) > 0])
        # Unrealized P&L (open trades)
        unrealized = sum(t.get("pnl_eur", 0) for t in self.open_trades)
        # Per pair breakdown
        per_pair = {}
        for pair in PIP.keys():
            pair_trades = [t for t in all_closed if t.get("pair") == pair]
            pair_wins = [t for t in pair_trades if t.get("pnl_eur", 0) > 0]
            per_pair[pair] = {
                "trades": len(pair_trades),
                "wr":     round(100 * len(pair_wins) / len(pair_trades), 1) if pair_trades else 0,
                "pnl":    round(sum(t.get("pnl_eur", 0) for t in pair_trades), 2),
            }
        return {
            "total_trades":   len(all_closed),
            "wins":           len(wins),
            "losses":         len(losses),
            "wr":             round(100 * len(wins) / len(all_closed), 1) if all_closed else 0,
...
def api_engine_start():
    ...
    cfg["lot_index"] = body.get("lot_index", cfg.get("lot_index", 1))
    cfg["lot_crypto"] = body.get("lot_crypto", cfg.get("lot_crypto", 1))
...
def api_engine_reset():
    """Full reset: stop engine, clear all trades + closed history + scan counter.
    Keeps config + presets. Trades worden in een backup-blob bewaard voor 1 day mocht je willen restoren.
    """
    body = request.json or {}
    confirm = body.get("confirm", False)
    if not confirm:
        return jsonify({"ok": False, "error": "Confirm flag required (confirm: true)"}), 400
    # Stop first
    if engine.running:
        engine.stop()
    ...
    try:
        backup_file = BASE_DIR / f"gamanx_backup_{int(now_brussels().timestamp())}.json"
        with open(backup_file, "w") as f:
            json.dump(backup, f, indent=2, default=str)
        log("INFO", f"Backup saved: {backup_file.name}")
    except Exception as e:
        log("WARN", f"Backup save failed: {e}")
    # Clear
    engine.open_trades = []
    engine.closed_trades = []
    engine.signal_history = {}
    engine.scan_count = 0
    engine.last_scan_ts = None
    engine._save_state()
    log("INFO", "ENGINE RESET — all trades + signal history cleared")
    return jsonify({"ok": True, "backup_file": backup_file.name if 'backup_file' in dir() else None})
...
def api_discord_test():
    """Stuur een test bericht naar de configured Discord webhook."""
    body = request.json or {}
    url = body.get("webhook_url") or engine.config.get("discord_webhook", "")
    if not url:
        return jsonify({"ok": False, "error": "Geen webhook URL ingesteld"}), 400
    try:
        msg = f"🔔 **GAMAN-X test bericht**\nTime: `{fmt_brussels()}`\nEngine: `{('running' if engine.running else 'stopped')}`\nOpen trades: `{len(engine.open_trades)}`\nIf you see this, Discord webhook works! ✅"
        r = requests.post(url, json={"content": msg, "username": "GAMAN-X"}, timeout=5)
        if r.status_code in (200, 204):
            return jsonify({"ok": True, "status_code": r.status_code})
        return jsonify({"ok": False, "error": f"Discord returned status {r.status_code}: {r.text[:200]}"}), 500
    except Exception as e:
...
@app.route("/api/trade/<tid>/delete", methods=["DELETE", "POST"])
def api_trade_delete(tid):
    """Delete a single trade from closed_trades history.
    Only works on closed trades — open trades must be closed first.
    """
    before = len(engine.closed_trades)
    engine.closed_trades = [t for t in engine.closed_trades if t.get("id") != tid]
    deleted = before - len(engine.closed_trades)
    if deleted == 0:
        return jsonify({"ok": False, "error": "Trade not found in history"}), 404
    engine._save_state()
    log("INFO", f"Trade {tid} deleted from history")
    return jsonify({"ok": True, "deleted": deleted})

@app.route("/api/trades/clear_closed", methods=["POST"])
def api_trades_clear_closed():
    """Bulk: wis alle closed_trades. Vereist confirm flag.
    Maakt automatisch backup zoals reset endpoint.
    """
    body = request.json or {}
    if not body.get("confirm"):
        return jsonify({"ok": False, "error": "Confirm flag required"}), 400
    count = len(engine.closed_trades)
    if count == 0:
        return jsonify({"ok": True, "deleted": 0})
    # Backup
    try:
        backup_file = BASE_DIR / f"gamanx_closed_backup_{int(now_brussels().timestamp())}.json"
        with open(backup_file, "w") as f:
            json.dump({"ts": fmt_brussels(), "closed_trades": list(engine.closed_trades)}, f, indent=2, default=str)
    except Exception as e:
        log("WARN", f"Backup failed: {e}")
        backup_file = None
    engine.closed_trades = []
    engine._save_state()
    log("INFO", f"Cleared {count} closed trades from history")
    return jsonify({"ok": True, "deleted": count, "backup_file": backup_file.name if backup_file else None})

@app.route("/api/trade/<tid>/edit", methods=["POST"])
def api_trade_edit(tid):
    """Edit SL en/of TP van een open trade."""
    body = request.json or {}
    trade = next((t for t in engine.open_trades if t["id"] == tid), None)
    if not trade:
        return jsonify({"ok": False, "error": "Trade not found"}), 404
    new_sl = body.get("sl")
    new_tp = body.get("tp")
    # Validatie: SL/TP moeten aan correcte zijde van entry liggen
    entry = trade["entry"]
    direction = trade["direction"]
    if new_sl is not None:
        new_sl = float(new_sl)
        if direction == "LONG" and new_sl >= entry:
            return jsonify({"ok": False, "error": "LONG: SL must be below entry"}), 400
        if direction == "SHORT" and new_sl <= entry:
            return jsonify({"ok": False, "error": "SHORT: SL must be above entry"}), 400
        trade["sl"] = new_sl
    if new_tp is not None:
        new_tp = float(new_tp)
        if direction == "LONG" and new_tp <= entry:
            return jsonify({"ok": False, "error": "LONG: TP must be above entry"}), 400
        if direction == "SHORT" and new_tp >= entry:
            return jsonify({"ok": False, "error": "SHORT: TP must be below entry"}), 400
        trade["tp"] = new_tp
    engine._save_state()
    log("INFO", f"Trade {tid} edited — SL={trade['sl']:.5f} TP={trade['tp']:.5f}")
...
<div class="card">
  <div class="card-title">Performance Overview <span id="ovLastUpdate" style="font-size:10px;color:var(--text3);font-weight:400;text-transform:none;letter-spacing:0">—</span></div>
  <!-- Top stats: 3 big tiles -->
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px">
    <div style="background:rgba(255,255,255,.03);border:1px solid var(--border);border-radius:6px;padding:14px 10px;text-align:center">
      <div id="ovTotalPnl" style="font-size:22px;font-weight:700;font-family:'JetBrains Mono',monospace">€0.00</div>
      <div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:2px;margin-top:4px">Total P&L</div>
    </div>
    <div style="background:rgba(255,255,255,.03);border:1px solid var(--border);border-radius:6px;padding:14px 10px;text-align:center">
      <div id="ovTodayPnl" style="font-size:22px;font-weight:700;font-family:'JetBrains Mono',monospace">€0.00</div>
      <div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:2px;margin-top:4px">Today P&L</div>
    </div>
    <div style="background:rgba(255,255,255,.03);border:1px solid var(--border);border-radius:6px;padding:14px 10px;text-align:center">
      <div id="ovWR" style="font-size:22px;font-weight:700;font-family:'JetBrains Mono',monospace">0%</div>
      <div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:2px;margin-top:4px">Win Rate</div>
    </div>
  </div>
  <!-- Secondary stats row: 4 -->
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:6px;margin-bottom:12px">
    <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:8px;text-align:center">
      <div id="ovTradeCount" style="font-size:14px;font-weight:600">0</div>
      <div style="font-size:9px;color:var(--text3);text-transform:uppercase">Trades</div>
    </div>
    <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:8px;text-align:center">
      <div id="ovPF" style="font-size:14px;font-weight:600">0.00</div>
      <div style="font-size:9px;color:var(--text3);text-transform:uppercase">Profit Factor</div>
    </div>
    <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:8px;text-align:center">
      <div id="ovMaxDD" style="font-size:14px;font-weight:600">€0</div>
      <div style="font-size:9px;color:var(--text3);text-transform:uppercase">Max DD</div>
    </div>
    <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:8px;text-align:center">
      <div id="ovUnrealized" style="font-size:14px;font-weight:600">€0</div>
      <div style="font-size:9px;color:var(--text3);text-transform:uppercase">Unrealized P&L</div>
    </div>
  </div>
  <!-- Per pair breakdown -->
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
    <div style="background:rgba(0,0,0,.2);border-radius:4px;padding:8px 10px;font-size:11px">
      <div style="color:var(--text3);font-size:9px;text-transform:uppercase;letter-spacing:1px;margin-bottom:3px">EURUSD</div>
      <div id="ovEurStats" style="font-family:'JetBrains Mono',monospace">0 tr · 0% WR · €0</div>
    </div>
    <div style="background:rgba(0,0,0,.2);border-radius:4px;padding:8px 10px;font-size:11px">
      <div style="color:var(--text3);font-size:9px;text-transform:uppercase;letter-spacing:1px;margin-bottom:3px">XAUUSD</div>
      <div id="ovXauStats" style="font-family:'JetBrains Mono',monospace">0 tr · 0% WR · €0</div>
    </div>
  </div>
  <!-- Per strategy breakdown -->
  <div style="font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Per Strategy</div>
  <div id="ovPerStrategy" style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:6px"></div>
</div>
...
<div class="card">
  <div class="card-title" style="display:flex;align-items:center;justify-content:space-between">
    ...
    <button class="btn danger" onclick="clearAllClosed()" style="padding:3px 10px;font-size:10px;font-weight:400;border-style:dashed;text-transform:none;letter-spacing:0">🗑 Clear all closed</button>
  </div>
...
<div class="card">
  ...
  <div style="display:flex;gap:8px;margin-bottom:8px">
    ...
    <button class="btn danger" onclick="engineReset()" style="border-style:dashed;margin-left:auto" title="Wis alle trades en reset stats">🗑 RESET</button>
  </div>
  
  <div class="form-row">
    <div class="form-group">
      <label>Pair</label>
      <div id="cfgPairChips" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px"><div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid var(--accent);background:rgba(0,255,180,.18);color:var(--accent);font-weight:600">EURUSD</div><div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">GBPUSD</div><div class="pair-chip active" data-pair="XAUUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid var(--accent);background:rgba(0,255,180,.18);color:var(--accent);font-weight:600">XAUUSD</div><div class="pair-chip" data-pair="SPX500" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">SPX500</div><div class="pair-chip" data-pair="US30" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">US30</div><div class="pair-chip" data-pair="BTCUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">BTCUSD</div><div class="pair-chip" data-pair="ETHUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">ETHUSD</div></div><input type="hidden" id="cfgPair" value="EURUSD">
    </div>
    <div class="form-group">
  ...
  <div class="form-row">
    <div class="form-group"><label>Lot Forex (EUR/GBP)</label><input type="number" id="cfgLotEur" value="10" min="1"></div>
    <div class="form-group"><label>Lot Metal (XAU)</label><input type="number" id="cfgLotXau" value="1" min="1"></div>
    <div class="form-group"><label>Lot Index (SPX/US30)</label><input type="number" id="cfgLotIndex" value="1" min="1"></div>
    <div class="form-group"><label>Lot Crypto (BTC/ETH)</label><input type="number" id="cfgLotCrypto" value="1" min="1"></div>
  </div>
  <div class="form-row">
  ...
  <div class="form-group" style="margin-bottom:10px">
    ...
    <div style="display:flex;gap:6px">
      <input type="text" id="cfgDiscord" placeholder="https://discord.com/api/webhooks/..." style="flex:1">
      <button class="btn" onclick="testDiscord()" style="white-space:nowrap;padding:8px 14px;font-size:11px">📨 Test</button>
    </div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="form-row">
    <div class="form-group"><label>Pair</label><div id="btPairChips" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px"><div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid var(--accent);background:rgba(0,255,180,.18);color:var(--accent);font-weight:600">EURUSD</div><div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">GBPUSD</div><div class="pair-chip" data-pair="XAUUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">XAUUSD</div><div class="pair-chip" data-pair="SPX500" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">SPX500</div><div class="pair-chip" data-pair="US30" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">US30</div><div class="pair-chip" data-pair="BTCUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">BTCUSD</div><div class="pair-chip" data-pair="ETHUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">ETHUSD</div></div><input type="hidden" id="btPair" value="EURUSD"></div>
...
// ── Pair chip helpers (shared between config + backtest) ──
function togglePairChip(el){
  const container = el.parentElement;
  el.classList.toggle("active");
  const on = el.classList.contains("active");
  if(on){
    el.style.background = "rgba(0,255,180,.18)";
  ...
  const active = [...container.querySelectorAll(".pair-chip.active")].map(c => c.dataset.pair);
  if(active.length === 0){ el.click(); return; }  // min 1
  // Sync hidden legacy input (first active, for backward compat)
  const hidden = container.parentElement.querySelector('input[type="hidden"]');
  if(hidden) hidden.value = active[0];
}
function getSelectedPairs(containerId){
  const cont = document.getElementById(containerId);
  if(!cont) return null;
  const active = [...cont.querySelectorAll(".pair-chip.active")].map(c => c.dataset.pair);
  return active.length ? active : ["EURUSD"];
}
function setSelectedPairs(containerId, pairs){
  const cont = document.getElementById(containerId);
  ...
  cont.querySelectorAll(".pair-chip").forEach(chip => {
    ...
    chip.style.background = on ? "rgba(0,255,180,.18)" : "rgba(255,255,255,.03)";
    chip.style.color = on ? "var(--accent)" : "#888";
    chip.style.borderColor = on ? "var(--accent)" : "#333";
...
function renderOverallStats(){
  const ov = (S.status && S.status.overall) || {};
  const setEl = (id, val) => { const el = document.getElementById(id); if(el) el.textContent = val; };
  const setElColored = (id, val, num) => {
    const el = document.getElementById(id); if(!el) return;
    el.textContent = val;
    el.style.color = num > 0 ? "var(--green)" : (num < 0 ? "var(--red)" : "var(--text)");
  };
  const totalPnl = ov.total_pnl || 0;
  const todayPnl = ov.today_pnl || 0;
  const unrealized = ov.unrealized_pnl || 0;
  const maxDD = ov.max_dd || 0;
  setElColored("ovTotalPnl", `€${totalPnl >= 0 ? "+" : ""}${totalPnl.toFixed(2)}`, totalPnl);
  setElColored("ovTodayPnl", `€${todayPnl >= 0 ? "+" : ""}${todayPnl.toFixed(2)}`, todayPnl);
  setEl("ovWR", `${(ov.wr || 0).toFixed(1)}%`);
  setEl("ovTradeCount", `${ov.total_trades || 0}`);
  setEl("ovPF", `${(ov.profit_factor || 0).toFixed(2)}`);
  setElColored("ovMaxDD", `€${maxDD.toFixed(0)}`, maxDD);
  setElColored("ovUnrealized", `€${unrealized >= 0 ? "+" : ""}${unrealized.toFixed(2)}`, unrealized);
  const wrEl = document.getElementById("ovWR");
  if(wrEl){
    if((ov.total_trades || 0) < 5) wrEl.style.color = "var(--text3)";
    else if(ov.wr >= 55) wrEl.style.color = "var(--green)";
    else if(ov.wr >= 45) wrEl.style.color = "var(--text)";
    else wrEl.style.color = "var(--red)";
  }
  const pp = ov.per_pair || {};
  const eur = pp.EURUSD || {trades:0,wr:0,pnl:0};
  const xau = pp.XAUUSD || {trades:0,wr:0,pnl:0};
  setEl("ovEurStats", `${eur.trades} tr · ${eur.wr.toFixed(1)}% WR · €${eur.pnl >= 0 ? "+" : ""}${eur.pnl.toFixed(2)}`);
  setEl("ovXauStats", `${xau.trades} tr · ${xau.wr.toFixed(1)}% WR · €${xau.pnl >= 0 ? "+" : ""}${xau.pnl.toFixed(2)}`);
  const stats = (S.status && S.status.stats) || {};
  const stratNames = {SB:"Silver Bullet", CH:"CHoCH", BOS:"BOS", AB:"Asian BO"};
  const stratEl = document.getElementById("ovPerStrategy");
  if(stratEl){
    stratEl.innerHTML = ["SB","CH","BOS","AB"].map(sid => {
      const s = stats[sid] || {trades:0,wr:0,pnl:0};
      const pnlColor = s.pnl > 0 ? "var(--green)" : (s.pnl < 0 ? "var(--red)" : "var(--text2)");
      return `
        <div style="background:rgba(0,0,0,.2);border-radius:4px;padding:5px 7px;font-size:9px">
          <div style="display:flex;justify-content:space-between;align-items:center">
            <span style="font-weight:600;color:var(--text)">${stratNames[sid]}</span>
            <span style="color:${pnlColor};font-weight:600">€${s.pnl >= 0 ? "+" : ""}${s.pnl.toFixed(2)}</span>
          </div>
          <div style="color:var(--text3);margin-top:1px">${s.trades} tr · ${s.wr.toFixed(1)}% WR</div>
        </div>
      `;
    }).join("");
  }
  const now = new Date();
  setEl("ovLastUpdate", `updated ${now.toTimeString().slice(0,8)}`);
}
      ...
      <tbody>${trades.map((t, idx) => `
        <tr class="live-tr" data-tidx="${idx}" style="cursor:pointer" onclick="toggleLiveTradeDetail('open', ${idx}, this, event)">
          ...
          <td style="white-space:nowrap">
            <button class="btn" style="padding:3px 6px;font-size:10px" onclick="event.stopPropagation();editTrade('${t.id}', ${t.sl}, ${t.tp}, '${t.direction}', '${t.pair}')" title="Edit SL/TP">✎</button>
            <button class="btn danger" style="padding:3px 6px;font-size:10px;margin-left:3px" onclick="event.stopPropagation();closeTrade('${t.id}')" title="Close trade">✕</button>
          </td>
        </tr>
      `).join("")}</tbody>
    </table>
...
function renderClosed(){
  ...
  document.getElementById("closedTrades").innerHTML = `
    <table>
      <thead><tr><th>Time</th><th>ID</th><th>Strat</th><th>TF</th><th>Pair</th><th>Dir</th><th>Entry</th><th>Exit</th><th>Hit</th><th>Pips</th><th>P&L</th><th></th></tr></thead>
      <tbody>${trades.map((t, idx) => `
        <tr class="live-tr-closed" data-tidx="${idx}" style="cursor:pointer" onclick="toggleLiveTradeDetail('closed', ${idx}, this, event)">
          ...
          <td class="${(t.pnl_eur||0)>=0?'pnl-pos':'pnl-neg'}">€${(t.pnl_eur||0).toFixed(2)}</td>
          <td><button class="btn danger" style="padding:2px 7px;font-size:10px" onclick="event.stopPropagation();deleteTrade('${t.id}')" title="Delete uit historiek">🗑</button></td>
        </tr>
      `).join("")}</tbody>
    </table>
...
function renderConfig(){
  ...
  if(S.config.pairs && S.config.pairs.length){
    setSelectedPairs("mCfgPairChips", S.config.pairs);
  } else if(S.config.pair === "BOTH"){
    setSelectedPairs("mCfgPairChips", ["EURUSD","XAUUSD"]);
  } else if(S.config.pair){
    setSelectedPairs("mCfgPairChips", [S.config.pair]);
  }
  if(S.config.pairs && S.config.pairs.length){
    setSelectedPairs("cfgPairChips", S.config.pairs);
  } else if(S.config.pair === "BOTH"){
    setSelectedPairs("cfgPairChips", ["EURUSD","XAUUSD"]);
  } else if(S.config.pair){
    setSelectedPairs("cfgPairChips", [S.config.pair]);
  }
  // Multi-TF chips
  ...
  if(document.getElementById("cfgLotIndex")) document.getElementById("cfgLotIndex").value = S.config.lot_index || 1;
  if(document.getElementById("cfgLotCrypto")) document.getElementById("cfgLotCrypto").value = S.config.lot_crypto || 1;
...
function getBodyConfig(){
  ...
  return {
    ...
    pairs: getSelectedPairs("cfgPairChips"),
    ...
    lot_index: parseInt((document.getElementById("cfgLotIndex")||{}).value)||1,
    lot_crypto: parseInt((document.getElementById("cfgLotCrypto")||{}).value)||1,
...
async function engineReset(){
  if(!confirm("⚠️ FULL RESET\n\nAlle open trades + closed history worden gewist.\nEngine wordt gestopt.\nBackup wordt automatisch opgeslagen.\n\nDoorgaan?")) return;
  if(!confirm("Echt zeker? Dit kan niet ongedaan worden gemaakt zonder backup file te herstellen.")) return;
  try{
    const r = await fetch("/api/engine/reset", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({confirm: true})
    });
    const d = await r.json();
    if(d.ok){
      alert("✅ Engine reset · backup: " + (d.backup_file || "—"));
      refreshStatus();
    } else {
...
async function editTrade(tid, currentSL, currentTP, direction, pair){
  const dec = pair === "XAUUSD" ? 2 : 5;
  const slStr = prompt(`Nieuwe SL voor ${direction} ${pair}\n(huidig: ${currentSL.toFixed(dec)})\nLOG: ${direction==="LONG"?"SL onder entry":"SL boven entry"}`, currentSL.toFixed(dec));
  if(slStr === null) return;
  const tpStr = prompt(`Nieuwe TP voor ${direction} ${pair}\n(huidig: ${currentTP.toFixed(dec)})\nLOG: ${direction==="LONG"?"TP boven entry":"TP onder entry"}`, currentTP.toFixed(dec));
  if(tpStr === null) return;
  const newSL = parseFloat(slStr.replace(",", "."));
  const newTP = parseFloat(tpStr.replace(",", "."));
  if(isNaN(newSL) || isNaN(newTP)){ alert("Ongeldige waardes"); return; }
  try{
    const r = await fetch(`/api/trade/${tid}/edit`, {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({sl: newSL, tp: newTP})
    });
    const d = await r.json();
    if(d.ok){
      alert("✅ SL/TP bijgewerkt");
      refreshStatus();
    } else {
      alert("Edit failed: " + (d.error || "?"));
    }
  }catch(e){ alert("Error: " + e.message); }
}

async function testDiscord(){
  const webhook = document.getElementById("cfgDiscord").value.trim();
  if(!webhook){ alert("Eerst webhook URL invoeren"); return; }
  try{
    ...
    const d = await r.json();
    if(d.ok){
      alert("✅ Discord test bericht verstuurd — check je channel");
    } else {
...
async function deleteTrade(tid){
  if(!confirm(`Trade ${tid} uit historiek verwijderen?\n\nDeze actie kan niet ongedaan worden gemaakt.`)) return;
  try{
    const r = await fetch(`/api/trade/${tid}/delete`, {method:"DELETE"});
    const d = await r.json();
    if(d.ok){
      refreshStatus();
    } else {
...
async function clearAllClosed(){
  const trades = (S.status && S.status.closed_trades) || [];
  if(trades.length === 0){ alert("Geen trades om te wissen"); return; }
  if(!confirm(`⚠️ Alle ${trades.length} closed trades uit historiek wissen?\n\nBackup wordt automatisch opgeslagen in C:\\gamanx\\.\nDeze actie heeft geen invloed op je open trades.\n\nDoorgaan?`)) return;
  try{
    const r = await fetch("/api/trades/clear_closed", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({confirm: true})
    });
    const d = await r.json();
    if(d.ok){
      alert(`✅ ${d.deleted} trades gewist · backup: ${d.backup_file || "—"}`);
      refreshStatus();
    } else {
      alert("Failed: " + (d.error || "?"));
    }
...
async function runBacktest(){
  ...
  const baseBody = {
    ...
    pairs: getSelectedPairs("btPairChips"),
...
<div class="tabpage active" id="page-live">
  ...
  <div class="card">
    <div class="card-title">Performance Overview <span id="ovLastUpdate" style="font-size:9px;color:var(--text3);font-weight:400;text-transform:none;letter-spacing:0">—</span></div>
    <!-- Top stats: 3 big tiles -->
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:10px">
      <div style="background:rgba(255,255,255,.03);border-radius:6px;padding:10px 6px;text-align:center">
        <div id="ovTotalPnl" style="font-size:16px;font-weight:700">€0.00</div>
        <div style="font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-top:2px">Total P&L</div>
      </div>
      <div style="background:rgba(255,255,255,.03);border-radius:6px;padding:10px 6px;text-align:center">
        <div id="ovTodayPnl" style="font-size:16px;font-weight:700">€0.00</div>
        <div style="font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-top:2px">Today P&L</div>
      </div>
      <div style="background:rgba(255,255,255,.03);border-radius:6px;padding:10px 6px;text-align:center">
        <div id="ovWR" style="font-size:16px;font-weight:700">0%</div>
        <div style="font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-top:2px">Win Rate</div>
      </div>
    </div>
    <!-- Secondary stats: 4 small tiles -->
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:4px;margin-bottom:10px">
      <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:6px 4px;text-align:center">
        <div id="ovTradeCount" style="font-size:12px;font-weight:600">0</div>
        <div style="font-size:8px;color:var(--text3)">Trades</div>
      </div>
      <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:6px 4px;text-align:center">
        <div id="ovPF" style="font-size:12px;font-weight:600">0.00</div>
        <div style="font-size:8px;color:var(--text3)">PF</div>
      </div>
      <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:6px 4px;text-align:center">
        <div id="ovMaxDD" style="font-size:12px;font-weight:600">€0</div>
        <div style="font-size:8px;color:var(--text3)">Max DD</div>
      </div>
      <div style="background:rgba(0,0,0,.3);border-radius:4px;padding:6px 4px;text-align:center">
        <div id="ovUnrealized" style="font-size:12px;font-weight:600">€0</div>
        <div style="font-size:8px;color:var(--text3)">Open P&L</div>
      </div>
    </div>
    <!-- Per pair breakdown -->
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px">
      <div style="background:rgba(0,0,0,.2);border-radius:4px;padding:6px 8px;font-size:10px">
        <div style="color:var(--text3);font-size:9px;text-transform:uppercase;letter-spacing:1px;margin-bottom:2px">EURUSD</div>
        <div id="ovEurStats">0 tr · 0% WR · €0</div>
      </div>
      <div style="background:rgba(0,0,0,.2);border-radius:4px;padding:6px 8px;font-size:10px">
        <div style="color:var(--text3);font-size:9px;text-transform:uppercase;letter-spacing:1px;margin-bottom:2px">XAUUSD</div>
        <div id="ovXauStats">0 tr · 0% WR · €0</div>
      </div>
    </div>
    <!-- Per strategy breakdown -->
    <div style="font-size:9px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Per Strategy</div>
    <div id="ovPerStrategy" style="display:grid;grid-template-columns:1fr 1fr;gap:4px"></div>
  </div>
  ...
  <div class="card">
    ...
    <div style="font-size:10px;color:var(--text3);font-family:'JetBrains Mono',monospace;text-align:center;margin-bottom:8px" id="scanInfo">Scans: 0</div>
    <button class="btn danger" onclick="engineReset()" style="font-size:10px;padding:8px;width:100%;border-style:dashed">🗑 FULL RESET (clear all trades)</button>
  </div>
  ...
  <div class="card">
    <div class="card-title">
      Recent Trades
      <button class="btn danger" onclick="clearAllClosed()" style="padding:3px 8px;font-size:9px;width:auto;font-weight:400;border-style:dashed">🗑 Clear all</button>
    </div>
...
<div class="tabpage" id="page-config">
  ...
  <div class="card collapse-section">
    ...
    <div class="collapse-body">
      <div class="form-row">
        <div class="form-group"><label>Pair</label><div id="mCfgPairChips" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px"><div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid var(--accent);background:rgba(0,255,180,.18);color:var(--accent);font-weight:600">EURUSD</div><div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">GBPUSD</div><div class="pair-chip active" data-pair="XAUUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid var(--accent);background:rgba(0,255,180,.18);color:var(--accent);font-weight:600">XAUUSD</div><div class="pair-chip" data-pair="SPX500" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">SPX500</div><div class="pair-chip" data-pair="US30" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">US30</div><div class="pair-chip" data-pair="BTCUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">BTCUSD</div><div class="pair-chip" data-pair="ETHUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">ETHUSD</div></div><input type="hidden" id="cfgPair" value="EURUSD"></div>
      ...
      <div class="form-row">
        <div class="form-group"><label>Lot Forex</label><input type="number" id="cfgLotEur" value="10" min="1"></div>
        <div class="form-group"><label>Lot Metal</label><input type="number" id="cfgLotXau" value="1" min="1"></div>
        <div class="form-group"><label>Lot Index</label><input type="number" id="cfgLotIndex" value="1" min="1"></div>
        <div class="form-group"><label>Lot Crypto</label><input type="number" id="cfgLotCrypto" value="1" min="1"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>Capital</label><input type="number" id="cfgCap" value="10000" min="100"></div>
        <div class="form-group"></div>
      ...
      <div class="form-group">
        ...
        <button class="btn" onclick="testDiscord()" style="font-size:10px;padding:6px;margin-top:6px;width:100%">📨 Test Discord webhook</button>
      </div>
    </div>
  </div>
...
<div class="tabpage" id="page-backtest">
  ...
  <div class="card">
    ...
    <div class="form-row">
      <div class="form-group"><label>Pair</label><div id="mBtPairChips" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px"><div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid var(--accent);background:rgba(0,255,180,.18);color:var(--accent);font-weight:600">EURUSD</div><div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">GBPUSD</div><div class="pair-chip" data-pair="XAUUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">XAUUSD</div><div class="pair-chip" data-pair="SPX500" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">SPX500</div><div class="pair-chip" data-pair="US30" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">US30</div><div class="pair-chip" data-pair="BTCUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">BTCUSD</div><div class="pair-chip" data-pair="ETHUSD" onclick="togglePairChip(this)" style="padding:6px 10px;border-radius:14px;font-size:11px;cursor:pointer;user-select:none;transition:.15s;border:1px solid #333;background:rgba(255,255,255,.03);color:#888">ETHUSD</div></div><input type="hidden" id="btPair" value="EURUSD"></div>
  ...
  el.innerHTML = `<div class="trades-list">${trades.map((t,idx) => `
    <div class="trade-row" style="cursor:pointer" onclick="toggleMobLiveTradeDetail('open', ${idx}, this, event)">
      ...
      <div class="row2">
        ...
        <div style="display:flex;gap:4px">
          <button class="btn" style="width:auto;padding:3px 8px;font-size:9px" onclick="event.stopPropagation();editTrade('${t.id}', ${t.sl}, ${t.tp}, '${t.direction}', '${t.pair}')">✎ Edit</button>
          <button class="btn danger" style="width:auto;padding:3px 8px;font-size:9px" onclick="event.stopPropagation();closeTrade('${t.id}')">✕ Close</button>
        </div>
      </div>
  ...
  el.innerHTML = `<div class="trades-list">${trades.map((t, idx) => `
    <div class="trade-row" style="cursor:pointer" onclick="toggleMobLiveTradeDetail('closed', ${idx}, this, event)">
      ...
      <div class="row2">
        ...
        <button class="btn danger" style="width:auto;padding:2px 7px;font-size:9px" onclick="event.stopPropagation();deleteTrade('${t.id}')" title="Delete">🗑</button>
      </div>
      <div class="trade-detail" style="display:none;margin-top:10px;padding-top:10px;border-top:1px solid var(--border)"></div>
    </div>
  `).join("")}</div>`;
...
function getBodyConfig(){
  ...
  return {
    pair: document.getElementById("cfgPair").value,
    pairs: getSelectedPairs("mCfgPairChips"),
    tfs: [...document.querySelectorAll("#cfgTFPicker .tf-chip.active")].map(c => c.dataset.tf),
    lot_eur: parseInt(document.getElementById("cfgLotEur").value)||10,
    lot_xau: parseInt(document.getElementById("cfgLotXau").value)||1,
    lot_index: parseInt((document.getElementById("cfgLotIndex")||{}).value)||1,
...
async function engineReset(){
  if(!confirm("⚠️ FULL RESET — alle open trades + closed history wissen.\n\nEngine wordt gestopt en alle trades cleared.\nBackup wordt automatisch opgeslagen.\n\nDoorgaan?")) return;
  if(!confirm("Echt zeker? Dit kan niet ongedaan worden gemaakt zonder backup file te herstellen.")) return;
  try{
    const r = await fetch("/api/engine/reset", {
      method:"POST", headers:{"Content-Type":"application/json"},
    ...
    const d = await r.json();
    if(d.ok){
      showToast("Engine reset · backup: " + (d.backup_file || "—"));
      refreshStatus();
    } else {
...
async function editTrade(tid, currentSL, currentTP, direction, pair){
  const dec = pair === "XAUUSD" ? 2 : 5;
  const slStr = prompt(`Nieuwe SL (${direction}, huidig: ${currentSL.toFixed(dec)}):`, currentSL.toFixed(dec));
  if(slStr === null) return;
  const tpStr = prompt(`Nieuwe TP (${direction}, huidig: ${currentTP.toFixed(dec)}):`, currentTP.toFixed(dec));
  if(tpStr === null) return;
  const newSL = parseFloat(slStr.replace(",", "."));
  const newTP = parseFloat(tpStr.replace(",", "."));
  if(isNaN(newSL) || isNaN(newTP)){ showToast("Ongeldige waardes"); return; }
  try{
    const r = await fetch(`/api/trade/${tid}/edit`, {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({sl: newSL, tp: newTP})
    ...
    const d = await r.json();
    if(d.ok){
      showToast("SL/TP bijgewerkt");
      refreshStatus();
    } else {
      showToast("Edit failed: " + (d.error || "?"));
    }
  }catch(e){ showToast("Error: " + e.message); }
}
...
async function testDiscord(){
  const webhook = document.getElementById("cfgDiscord").value.trim();
  if(!webhook){ showToast("Eerst webhook URL invoeren"); return; }
  showToast("Test bericht verzenden...");
  try{
    const r = await fetch("/api/discord/test", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({webhook_url: webhook})
    ...
    const d = await r.json();
    if(d.ok){
      showToast("✅ Discord bericht verstuurd");
    } else {
      showToast("❌ Failed: " + (d.error || "?").slice(0, 60));
    }
  }catch(e){ showToast("Error: " + e.message); }
}

async function deleteTrade(tid){
  if(!confirm(`Trade ${tid} uit historiek verwijderen?`)) return;
  try{
    const r = await fetch(`/api/trade/${tid}/delete`, {method:"DELETE"});
    const d = await r.json();
    if(d.ok){
...
async function clearAllClosed(){
  const trades = (S.status && S.status.closed_trades) || [];
  if(trades.length === 0){ showToast("Geen trades om te wissen"); return; }
  if(!confirm(`⚠️ Alle ${trades.length} closed trades uit historiek wissen?\n\nBackup wordt automatisch opgeslagen.`)) return;
  try{
    const r = await fetch("/api/trades/clear_closed", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({confirm: true})
    ...
    const d = await r.json();
    if(d.ok){
      showToast(`✅ ${d.deleted} trades gewist`);
      refreshStatus();
    } else {
      showToast("Failed: " + (d.error || "?"));
    }
...
async function runBacktest(){
  ...
  const baseBody = {
    ...
    pair: document.getElementById("btPair").value,
    pairs: getSelectedPairs("mBtPairChips"),
    start: document.getElementById("btStart").value,
    end: document.getElementById("btEnd").value,
    lot: parseInt(document.getElementById("btLot").value)||10,
    rr: parseFloat(document.getElementById("btRR").value)||2,
