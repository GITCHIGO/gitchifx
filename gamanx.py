...
class GamanXEngine:
            ...
            for tf in tfs:
                for pair in pairs:
                    try:
                        ...
                        if signal:
                            # Dedup checks (robuster):
                            #  1. Per strategy+pair+tf: same setup binnen 30 min
                            #  2. Per strategy+pair op zelfde entry level (±2 pip): voorkomt duplicate orders
                            #     ongeacht TF (BOS-001/002/004 bug)
                            #  3. Open trade van zelfde strategy+pair+direction: niet 2x stacking
                            if self._has_recent_trade(sid, pair, tf=tf, minutes=30):
                                continue
                            if self._has_open_similar_trade(sid, pair, signal["direction"], signal["entry"], pair_eps_pips=3):
                                log("INFO", f"Skip {sid} {pair} {tf}: similar trade already open at {signal['entry']:.5f}")
                                continue
    ...
    def _has_open_similar_trade(self, sid, pair, direction, entry, pair_eps_pips=3):
        ...
        en entry binnen `pair_eps_pips` pip range. Voorkomt duplicate orders
        die hetzelfde signaal vertegenwoordigen op verschillende TFs of scan cycles.
        """
        pip_size = PIP.get(pair, 0.0001)
...
def api_engine_reset():
    ...
    engine._save_state()
    log("INFO", "ENGINE RESET — all trades cleared")
