...
class LiveEngine:
    ...
    def _scan(self, pair_override=None, tf_override=None):
        ...
        auto_sltp    = bool(cfg.get("auto_sltp", False))              # automatische SL/TP berekening
        rr           = float(cfg.get("rr", 2.0))                      # alleen gebruikt als auto_sltp aan staat
        # Lotsize per pair
        if pair == "XAUUSD":
        ...
        # Automatische SL/TP berekening (als toggle aan staat)
        auto_sl = None
        auto_tp = None
        sl_method = None
        sl_pips_calc = 0
        if auto_sltp:
            ...
            if auto_sl is None:
                ...
                self.log("WARN", f"Auto SL/TP berekening mislukt voor {pair} {tf} ({sl_method}) — trade overgeslagen")
                return
        ...
        trade = {
            ...
            "sl":          auto_sl,   # automatisch berekend OF None (manueel)
            "tp":          auto_tp,   # automatisch berekend OF None (manueel)
...
def compute_sl_tp(df, fvg, direction, entry, pair, rr=2.0):
    ...
    SL prioriteit:
      ...
      2. Laagste low / hoogste high van laatste 10 bars vóór FVG, met buffer
      3. ATR-gebaseerde fallback (1.5× ATR) — past zich aan volatiliteit aan
      4. Veiligheid: minimum buffer per pair
    ...
    Returns: (sl, tp, sl_pips, sl_method)
      ...
      sl_method: string voor logging ("swing", "recent_low", "atr_fallback", "hard_fallback")
    """
    pip_v = PIP.get(pair, 0.0001)
    ...
    if direction == "LONG":
        ...
        # Methode 2: laagste low in laatste 10 bars vóór FVG
        ...
        # Veiligheid: zorg dat SL minimaal hard_min_pips weg is
        min_sl = entry - pip_v * hard_min_pips
        if sl > min_sl:  # te dicht
            # SL is verder dan entry maar te dicht — gebruik hard minimum
...
def engine_set_sl_tp():
    ...
    # Bepaal live prijs voor validatie (buiten lock zodat fetch_price niet blokkeert)
    ...
    if not live_price:
        try:
            ...
            live_price = None
    # Fallback naar entry als geen live prijs beschikbaar
    if not live_price or live_price <= 0:
        live_price = entry

    with engine.lock:
        # Re-fetch trade in lock voor de write
        trade = next((t for t in engine.open_trades if t["id"] == trade_id), None)
        if not trade:
            return jsonify({"ok": False, "error": "Trade niet gevonden"})

        # Validatie tegen LIVE prijs — moet SL/TP aan de juiste kant zijn van waar prijs NU staat
    ...
    # Geen Discord notificatie bij handmatige wijziging — jij doet het zelf, te veel ruis anders
    engine._save_state()
...
:root{
  ...
  --border:#1f3a5e;
  --border2:#2b5a7a;
  --glow:#22d3ee;        /* electric cyan — voor accenten en actieve states */
  --glow2:#7dd3fc;       /* ice blue — hoofdkleur */
  --glow3:#bae6fd;       /* lichte ice blue voor tekst en subtiel */
  --text:#e0f2fe;
  --text2:#9bbfdc;
  --text3:#4e6c80;
  --green:#34d399;
...
body::before{
  ...
  background:radial-gradient(ellipse 90% 60% at 50% -10%,rgba(34,211,238,.30) 0%,transparent 65%),
             radial-gradient(ellipse 50% 40% at 85% 90%,rgba(125,211,252,.18) 0%,transparent 55%),
             radial-gradient(ellipse 40% 30% at 5% 70%,rgba(34,211,238,.13) 0%,transparent 50%);
...
body::after{
  content:'';position:fixed;inset:0;
  background:repeating-linear-gradient(0deg,rgba(34,211,238,.025) 0px,rgba(34,211,238,.025) 1px,transparent 1px,transparent 4px);
...
/* Toggle switches */
.toggle-section{margin-top:10px;padding:10px 12px;background:rgba(34,211,238,.04);border:1px solid var(--border);border-radius:8px}
    ...
    <div class="card" id="live-engine-config">
      ...
      <div class="card-body">
        ...
        <div class="toggle-section">
          ...
          <div class="toggle-row">
            <div class="toggle-label">Automatische SL/TP berekening<small>Swing-based SL + RR multiple TP. Per-pair buffers (EUR/XAU).</small></div>
            <label class="switch"><input type="checkbox" id="lt-auto-sltp"><span class="slider"></span></label>
          </div>
          <div class="form-group" style="margin-top:10px">
            <label>Risk:Reward Ratio</label>
            <input type="number" id="lt-rr" value="2" min="0.5" max="10" step="0.1" title="Alleen actief als Auto SL/TP aan staat">
          </div>
          <div style="font-size:10px;color:var(--text3);line-height:1.4;padding:6px 8px;background:rgba(34,211,238,.05);border-radius:6px">
            ℹ️ Buffers per pair: <b>EUR</b> 3p swing / 5p recent / 20p hard fallback · <b>XAU</b> 30p / 50p / 150p. ATR-fallback voor onbekende condities. Je kunt SL/TP achteraf nog manueel aanpassen.
          </div>
        </div>
...
function buildModalChart(trade){
  ...
  if(!allData||!allData.length){ el.innerHTML='<div style="padding:20px;color:#7dd3fc;font-size:12px">Geen candledata beschikbaar — run de backtest opnieuw</div>'; return; }
  ...
  if(!filtered.length){ el.innerHTML='<div style="padding:20px;color:#7dd3fc;font-size:12px">Geen data in dit venster</div>'; return; }
...
function renderPresets(presets){
  ...
  el.innerHTML = names.map(n=>`
    <div style="display:flex;align-items:center;gap:6px;padding:5px 8px;border:1px solid var(--border);border-radius:6px;background:rgba(8,5,24,.6)">
      ...
      <button onclick="loadPresetIntoForm('${n}')" style="padding:2px 8px;border-radius:4px;border:1px solid var(--border2);background:rgba(34,211,238,.15);color:var(--glow2);font-size:10px;cursor:pointer">Laden</button>
...
document.addEventListener("DOMContentLoaded",()=>{
  lucide.createIcons();
  try{initChart()}catch(e){document.getElementById("chart").innerHTML=`<div style="padding:40px;color:#7dd3fc;font-family:Inter,sans-serif">Chart fout: ${e.message}</div>`}
...
:root {
  --bg:#04020f; --bg2:#0a1428; --card:rgba(10,20,40,.85);
  --border:rgba(31,58,94,.5); --border2:rgba(34,211,238,.35);
  --text:#e0f2fe; --text2:#7dd3fc; --text3:#4e6c80;
  --glow:#22d3ee; --glow2:#7dd3fc; --glow3:#bae6fd;
  --green:#22c55e; --red:#f87171; --amber:#fbbf24;
}
body {
  margin:0; padding:0;
  background: radial-gradient(ellipse at top, #0c1f3d 0%, #04020f 60%);
  color:var(--text);
...
.logo {
  ...
  text-shadow:0 0 12px rgba(34,211,238,.6), 0 0 24px rgba(34,211,238,.3);
  letter-spacing:1px;
  animation:logo-pulse 4s ease-in-out infinite;
}
@keyframes logo-pulse{
  0%,100%{text-shadow:0 0 10px rgba(34,211,238,.5), 0 0 20px rgba(34,211,238,.25)}
  50%{text-shadow:0 0 16px rgba(125,211,252,.7), 0 0 32px rgba(34,211,238,.4)}
}
.logo span { 
...
<div class="page active" id="page-live">
  ...
  <div class="card">
    ...
    <div class="card-body collapsed" id="m-cfg-body">
      ...
      <div class="toggle-row">
        <div class="toggle-info">
          <div class="toggle-name">Automatische SL/TP</div>
      ...
      <div style="font-size:10px;color:var(--text3);line-height:1.4;padding:8px;background:rgba(34,211,238,.05);border-radius:6px;margin-top:6px">
        ℹ️ EUR buffers: 3/5/20p. XAU: 30/50/150p. SL/TP achteraf nog manueel aanpasbaar.
      </div>
...
<div class="page" id="page-system">
  ...
  <div class="card">
    <div class="card-header no-toggle">
      ...
      <button onclick="mLoadHealth()" style="padding:6px 12px;border:1px solid var(--border2);border-radius:6px;background:rgba(34,211,238,.1);color:var(--glow2);font-size:11px;cursor:pointer">↻ Ververs</button>
    </div>
