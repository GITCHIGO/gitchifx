...
class LiveEngine:
    ...
    def _scan(self, pair_override=None, tf_override=None):
        ...
        # Entry prijs = midpunt FVG
        entry = (fvg["top"] + fvg["bottom"]) / 2
        ref_top = ob["top"]    if ob else fvg["top"]
        ref_bot = ob["bottom"] if ob else fvg["bottom"]
        ...
        self._discord(
            ...
            f"Bias Score: {score} | Lot: {lotsize}",
            0x7c3aed
        )
        self._save_state()
...
def detect_fvg(df, i):
    ...
    if l0 > h2: return {"type":"bull","top":l0,"bottom":h2,"formed_at":i}
    if h0 < l2: return {"type":"bear","top":l2,"bottom":h0,"formed_at":i}
    return None
...
def run_backtest(pair, tf, start, end, capital, lotsize, rr, use_ob, use_trend, use_eq, min_score, use_session=False, be_trigger=0.0, spread_pips=0.0, slippage_pips=0.0, lotsize_eur=None, lotsize_xau=None, max_daily_loss=0, max_trades=0, max_risk_pct=0, **kwargs):
    ...
    # Pre-scan alle FVGs en OBs eenmalig
    all_fvgs = {}  # formed_at -> fvg dict
    all_obs  = {}  # bar_index -> ob dict
    for i in range(2, n):
        f = detect_fvg(df, i)
        if f: all_fvgs[i] = f
    for i in range(1, n):
    ...
    for i in range(20, n - 2):
        ...
        # ── SL en TP (op basis van RAW prijs, voor spread) ──
        ref_top = ob["top"]    if ob else fvg["top"]
        ref_bot = ob["bottom"] if ob else fvg["bottom"]

        raw_entry = entry_price  # prijs zoals de markt hem ziet
      ...
      <div class="card-body">
        ...
        <div class="toggle-section">
          ...
          <div class="form-group" style="margin-top:6px">
            ...
            <input type="text" id="lt-discord" value="" placeholder="https://discord.com/api/webhooks/..." style="font-size:10px">
          </div>
...
function renderEngineStatus(d){
  ...
  if(!d.closed_trades || !d.closed_trades.length){
    ctb.innerHTML='<tr><td colspan="9" style="text-align:center;padding:16px;color:var(--text3)">Nog geen gesloten trades</td></tr>';
  } else {
    ctb.innerHTML = [...d.closed_trades].reverse().map(t=>{
      ...
      return `<tr>
        ...
        <td class="${w?"win":"loss"}">${(t.pips||0)>0?"+":""}${t.pips||0}</td>
        <td class="${w?"win":"loss"}">${(t.pnl_eur||0)>=0?"+":""}€${(t.pnl_eur||0).toFixed(2)}</td>
