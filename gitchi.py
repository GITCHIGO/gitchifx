...
class MT5Bridge:
    ...
    def place_order(self, trade):
        ...
        if not self.connected:
            self.log_fn("WARN", "MT5 EA offline — order niet verzonden")
            return None
        try:
            pair = trade["pair"]
    ...
    def close_order(self, trade_id, reason="MANUAL"):
        ...
        if not self.connected:
            self.log_fn("WARN", "MT5 EA offline — close niet verzonden")
            return False
        with self.lock:
...
def api_mt5_connect():
    # File-based bridge: geen actieve connect nodig, EA schrijft heartbeat autonoom
    connected = mt5_bridge.connected
    if not connected:
        return jsonify({"ok": False, "error": "Geen heartbeat van EA. Check: 1) MT5 desktop open? 2) GAMAN_Bridge EA op chart? 3) AutoTrading aan?", "connected": False}), 400
...
def api_mt5_toggle():
    ...
    if enabled and not mt5_bridge.connected:
        engine.log("WARN", "[MT5] Toggle aan maar EA offline — check MT5 desktop app + AutoTrading")
...
:root{
  ...
  --border:#1f3a5e;
  --border2:#2b5a7a;
  --glow:#22d3ee;        /* electric cyan — for accenten en actieve states */
  --glow2:#7dd3fc;       /* ice blue — hoofdkleur */
  --glow3:#bae6fd;       /* lichte ice blue for tekst en subtiel */
  --text:#e0f2fe;
  --text2:#9bbfdc;
  --text3:#4e6c80;
  --green:#34d399;
    ...
    <div class="card" id="live-engine-config">
      ...
      <div class="card-body">
        ...
        <div class="toggle-section collapse-section collapsed">
          <div class="toggle-section-title collapse-head" onclick="this.parentElement.classList.toggle('collapsed')">Discord Notificaties</div>
        ...
        <div class="toggle-section collapse-section collapsed" id="mt5-section">
          ...
          <div style="font-size:9px;color:var(--text3);margin-top:6px;line-height:1.4">
            Vereist: MT5 open + ingelogd + GAMAN_Bridge EA op chart + AutoTrading aan
          </div>
          </div>
        </div>
    ...
    <div class="card" id="bt-form" style="display:none">
      ...
      <div class="card-body">
        <div class="form-grid">
          ...
          <div class="form-group"><label>Max risico per trade (%)</label><input type="number" id="bt-risk-pct" value="0" min="0" max="25" step="0.5" title="0 = vaste lotsize"></div>
        </div>
...
async function toggleMT5(enabled){
  try{
    ...
    if(enabled && !d.connected){
      alert("MT5 kon niet verbinden — is MT5 desktop app open en ingelogd? Check logs.");
    }
...
async function toggleMT5Mobile(enabled){
  try{
    ...
    if(enabled && !d.connected){
      alert("MT5 kon niet verbinden — is MT5 open + ingelogd?");
    }
  }catch(e){}
}
...
document.addEventListener("DOMContentLoaded",()=>{
  lucide.createIcons();
  try{initChart()}catch(e){document.getElementById("chart").innerHTML=`<div style="padding:40px;color:#7dd3fc;font-family:Inter,sans-serif">Chart error: ${e.message}</div>`}
