...
def capture_trade_context(df, pair, tf, direction, entry, fvg, bias, score,
                          sl_pips, engine_ref=None):
    """Capture a rich snapshot of market conditions at trade OPEN.
    Returns a flat dict of ~45 fields for later analysis / ML optimization.
    Defensive: every field wrapped so a failure never blocks the trade."""
    import math
    ctx = {}
    try:
        ...
        # ── TIME CONTEXT ──
        ctx["ctx_hour"] = now_b.hour
        ctx["ctx_minute"] = now_b.minute
        ctx["ctx_weekday"] = now_b.strftime("%A")
        ctx["ctx_weekday_num"] = now_b.weekday()  # 0=Mon
        ctx["ctx_day"] = now_b.day
        ctx["ctx_month"] = now_b.month
        h = now_b.hour
        if 0 <= h < 8:      sess = "Asian"
        elif 8 <= h < 13:   sess = "London"
        elif 13 <= h < 16:  sess = "London-NY-overlap"
        elif 16 <= h < 21:  sess = "NY"
        else:               sess = "After-hours"
        ctx["ctx_session"] = sess
        ctx["ctx_killzone"] = (9 <= h < 12) or (14 <= h < 17)
        ctx["ctx_is_friday"] = (now_b.weekday() == 4)
        ctx["ctx_is_monday"] = (now_b.weekday() == 0)

        # ── VOLATILITY CONTEXT ──
        atr14 = calc_atr(df, period=14)
        ctx["ctx_atr14_pips"] = round(atr14 / pip_size, 1)
        # ATR percentile vs last 100 candles
        if n >= 100:
            atr_series = []
            ...
            if atr_series:
                cur_tr = highs[-1]-lows[-1]
                below = sum(1 for x in atr_series if x < cur_tr)
                ctx["ctx_atr_percentile"] = round(100 * below / len(atr_series), 0)
            else:
                ctx["ctx_atr_percentile"] = None
        else:
            ctx["ctx_atr_percentile"] = None
        # Candle bodies
        bodies = [abs(closes[i]-opens[i]) for i in range(max(0,n-10), n)]
        ctx["ctx_avg_body_pips_10"] = round((sum(bodies)/len(bodies))/pip_size, 1) if bodies else 0
        ctx["ctx_current_body_pips"] = round(abs(closes[-1]-opens[-1])/pip_size, 1)
        recent20 = [highs[i]-lows[i] for i in range(max(0,n-20), n)]
        ctx["ctx_largest_range_pips_20"] = round(max(recent20)/pip_size, 1) if recent20 else 0
        # Day range (last ~96 15M candles = 24h, approximate with last 96 bars)
        lookback_day = min(96, n)
        day_high = float(highs[n-lookback_day:n].max())
        day_low = float(lows[n-lookback_day:n].min())
        ctx["ctx_day_range_pips"] = round((day_high-day_low)/pip_size, 1)
        # Week range (last ~480 bars approx)
        lookback_week = min(480, n)
        week_high = float(highs[n-lookback_week:n].max())
        week_low = float(lows[n-lookback_week:n].min())
        ctx["ctx_week_range_pips"] = round((week_high-week_low)/pip_size, 1)
        # Position in day/week range (%)
        cur = closes[-1]
        ctx["ctx_pos_in_day_range"] = round(100*(cur-day_low)/(day_high-day_low), 0) if day_high > day_low else 50
        ctx["ctx_pos_in_week_range"] = round(100*(cur-week_low)/(week_high-week_low), 0) if week_high > week_low else 50
        ctx["ctx_dist_to_day_high_pips"] = round((day_high-cur)/pip_size, 1)
        ctx["ctx_dist_to_day_low_pips"] = round((cur-day_low)/pip_size, 1)

        # ── TREND & STRUCTURE ──
        def trend_of(sub):
            if len(sub) < 20: return "ranging"
            recent_h = sub[-10:].max(); prev_h = sub[-20:-10].max()
            return recent_h, prev_h
        # Simplified swing-based trend on current TF
        if n >= 20:
            rh = float(highs[-10:].max()); ph = float(highs[-20:-10].max())
            rl = float(lows[-10:].min()); pl = float(lows[-20:-10].min())
            hh = rh > ph; hl = rl > pl
            lh = rh < ph; ll = rl < pl
            ctx["ctx_trend_current"] = "up" if (hh and hl) else ("down" if (lh and ll) else "ranging")
        else:
            ctx["ctx_trend_current"] = "ranging"
        ctx["ctx_htf_bias"] = bias.get("verdict", "?")
        ctx["ctx_bias_score"] = score

        # ── FVG SETUP ──
        fvg_size = fvg["top"] - fvg["bottom"]
        ctx["ctx_fvg_size_pips"] = round(fvg_size/pip_size, 1)
        ctx["ctx_fvg_age_bars"] = (n-1) - fvg.get("formed_at", n-1)
        fvg_mid = (fvg["top"]+fvg["bottom"])/2
        ctx["ctx_entry_vs_fvg_mid_pips"] = round((entry-fvg_mid)/pip_size, 1)
        # Equilibrium position
        lb = min(20, n)
        eq = (float(highs[n-lb:n].max()) + float(lows[n-lb:n].min())) / 2
        ctx["ctx_fvg_vs_equilibrium"] = "below" if fvg_mid < eq else "above"

        # ── SCORE BREAKDOWN (judges) ──
        for j in ["j1","j2","j3","j4","j5"]:
            ctx[f"ctx_{j}"] = bias.get(j, 0)
            ctx[f"ctx_{j}_label"] = bias.get(f"{j}_label", "")
        aligned = sum(1 for j in ["j1","j2","j3","j4","j5"]
                      if (direction=="LONG" and bias.get(j,0)==1) or (direction=="SHORT" and bias.get(j,0)==-1))
        ctx["ctx_judges_aligned"] = aligned

        # ── CORRELATION & TRADING CONTEXT ──
        if engine_ref is not None:
            try:
                ctx["ctx_open_trades_count"] = len(engine_ref.open_trades)
                today = now_b.date()
                today_closed = [t for t in engine_ref.closed_trades
                                if t.get("opened_at","").startswith(str(today))]
                ctx["ctx_trades_today"] = len(today_closed)
                ctx["ctx_daily_pnl_so_far"] = round(sum(t.get("pnl_eur",0) for t in today_closed), 2)
                if engine_ref.closed_trades:
                    last = engine_ref.closed_trades[-1]
                    lpnl = last.get("pnl_eur", 0)
                    ctx["ctx_last_trade_result"] = "W" if lpnl > 0 else ("L" if lpnl < 0 else "BE")
                else:
                    ctx["ctx_last_trade_result"] = "none"
            except Exception:
                pass

        # ── NEWS PROXIMITY (Forex Factory events — high-impact within window) ──
        try:
            events = _fetch_news_raw()  # live FF weekly events (cached 30min)
            high_today = False
            high_within_60min = False
            if events:
                now_ts = now_b.timestamp()
                for ev in events:
                    impact = str(ev.get("impact", "")).lower()
                    if impact != "high":
                        continue
                    # FF date is ISO like "2026-01-15T14:30:00-05:00"
                    ev_date_str = str(ev.get("date", ""))
                    if ev_date_str[:10] == str(now_b.date()):
                        high_today = True
                    # Time proximity
                    try:
                        from datetime import datetime as _dt
                        ev_dt = _dt.fromisoformat(ev_date_str.replace("Z", "+00:00"))
                        diff_min = abs((ev_dt.timestamp() - now_ts) / 60)
                        if diff_min <= 60:
                            high_within_60min = True
                    except Exception:
                        pass
            ctx["ctx_high_impact_news_today"] = high_today
            ctx["ctx_high_impact_within_60min"] = high_within_60min
        except Exception:
            ctx["ctx_high_impact_news_today"] = None
            ctx["ctx_high_impact_within_60min"] = None

        # ── SPREAD AT ENTRY (from MT5 heartbeat if available, else estimate) ──
        try:
            spread_pips = None
            hb = mt5_bridge.account_info() if 'mt5_bridge' in globals() else None
            # Heartbeat doesn't carry per-symbol spread; approximate from recent candle
            # wick behaviour as a fallback proxy (not exact, but directionally useful)
            if spread_pips is None:
                # Estimate: typical IC Markets Raw spread by instrument
                est = {"EURUSD": 0.3, "GBPUSD": 0.6, "XAUUSD": 2.0}.get(pair, 1.0)
                spread_pips = est
            ctx["ctx_spread_at_entry_pips"] = round(spread_pips, 2)
        except Exception:
            ctx["ctx_spread_at_entry_pips"] = None

        # ── DISTANCE TO ROUND NUMBERS ──
        try:
            # For FX: round numbers at .00 and .50 levels (e.g. 1.1400, 1.1450)
            # For XAU: at whole 10s (e.g. 2650, 2660)
            if pair == "XAUUSD":
                round_step = 10.0
            else:
                round_step = 0.0050  # 50-pip levels
            nearest_round = round(entry / round_step) * round_step
            dist_round = abs(entry - nearest_round)
            ctx["ctx_dist_to_round_pips"] = round(dist_round / pip_size, 1)
            # Also flag the "big figure" (.00) distance
            big_step = 10.0 if pair == "XAUUSD" else 0.0100  # 100-pip
            nearest_big = round(entry / big_step) * big_step
            ctx["ctx_dist_to_big_figure_pips"] = round(abs(entry - nearest_big) / pip_size, 1)
        except Exception:
            ctx["ctx_dist_to_round_pips"] = None
            ctx["ctx_dist_to_big_figure_pips"] = None

        # ── MOMENTUM STREAK (consecutive same-color candles before entry) ──
        try:
            streak = 0
            streak_dir = None
            for i in range(n-1, max(0, n-11), -1):  # look back up to 10 candles
                is_green = closes[i] > opens[i]
                is_red = closes[i] < opens[i]
                if streak_dir is None:
                    if is_green: streak_dir = "green"; streak = 1
                    elif is_red: streak_dir = "red"; streak = 1
                    else: break
                elif (streak_dir == "green" and is_green) or (streak_dir == "red" and is_red):
                    streak += 1
                else:
                    break
            ctx["ctx_momentum_streak"] = streak
            ctx["ctx_momentum_direction"] = streak_dir or "none"
            # Streak aligned with trade direction?
            ctx["ctx_streak_aligns_trade"] = (
                (direction == "LONG" and streak_dir == "green") or
                (direction == "SHORT" and streak_dir == "red")
            )
        except Exception:
            ctx["ctx_momentum_streak"] = None
            ctx["ctx_momentum_direction"] = None
            ctx["ctx_streak_aligns_trade"] = None

        # ── DXY EXACT VALUE + CHANGE ──
        try:
            dxy_df = get_dxy_1h()
            if dxy_df is not None and len(dxy_df) >= 2:
                dxy_now = float(dxy_df["close"].iloc[-1])
                dxy_prev = float(dxy_df["close"].iloc[-2])
                ctx["ctx_dxy_value"] = round(dxy_now, 3)
                ctx["ctx_dxy_change_pct"] = round(100 * (dxy_now - dxy_prev) / dxy_prev, 3)
                ctx["ctx_dxy_direction"] = "up" if dxy_now > dxy_prev else ("down" if dxy_now < dxy_prev else "flat")
            else:
                ctx["ctx_dxy_value"] = None
                ctx["ctx_dxy_change_pct"] = None
                ctx["ctx_dxy_direction"] = None
        except Exception:
            ctx["ctx_dxy_value"] = None
            ctx["ctx_dxy_change_pct"] = None
            ctx["ctx_dxy_direction"] = None
        ...
        # ── SETUP METADATA ──
        ctx["ctx_sl_pips"] = sl_pips
    except Exception as e:
        ctx["ctx_capture_error"] = str(e)
    return ctx
...
class LiveEngine:
    def __init__(self):
        ...
        self.scan_status = {}     # pair+tf -> last reject reason (diagnostics)
        self._load_state()
    ...
    def _run(self):
        while self.running:
            ...
            try:
                ...
                # Determine pairs and timeframes
                max_trades = int(self.config.get("max_trades", 0))
                tf_map = {"15M+1H":["15M","1H"],"1H+4H":["1H","4H"],"ALL":["15M","1H","4H"]}
                # Multi-pair: "pairs" list (new) with fallback to legacy "pair" string
                cfg_pairs = self.config.get("pairs")
                if not cfg_pairs or not isinstance(cfg_pairs, list):
                    cfg_pair = self.config.get("pair","EURUSD")
                    if cfg_pair == "BOTH" or self.config.get("trade_both"):
                        cfg_pairs = ["EURUSD","XAUUSD"]
                    else:
                        cfg_pairs = [cfg_pair]
                pairs = [p for p in cfg_pairs if p in PIP]  # sanity filter
                if not pairs: pairs = ["EURUSD"]
    ...
    def _scan_diag(self, pair, tf, reason):
        """Scan diagnostics: log reject reason, but only when it CHANGES
        for this pair+tf (prevents log spam at 3 scans/min)."""
        key = f"{pair}_{tf}"
        if self.scan_status.get(key) != reason:
            self.scan_status[key] = reason
            self.log("SCAN", f"{pair} {tf}: {reason}")
        return None
    ...
    def _scan(self, pair_override=None, tf_override=None):
        ...
        # Lotsize per pair — each pair can have its own micro lot config
        if pair == "XAUUSD":
            lotsize = float(cfg.get("lotsize_xau", cfg.get("lotsize", 1)))
        elif pair == "GBPUSD":
            lotsize = float(cfg.get("lotsize_gbp", cfg.get("lotsize_eur", cfg.get("lotsize", 1))))
        else:  # EURUSD (default)
        ...
        if pair in open_pairs:
            return self._scan_diag(pair, tf, "blocked — open trade on this pair")
        ...
        if time.time() - last_entry < tf_seconds:
            remaining = int(tf_seconds - (time.time() - last_entry))
            return self._scan_diag(pair, tf, f"cooldown — {remaining}s remaining")
        ...
        if abs(score) < min_sc:
            return self._scan_diag(pair, tf, f"score {score:+d} < min ±{min_sc} — no direction")

        direction = "LONG" if score >= min_sc else ("SHORT" if score <= -min_sc else None)
        if direction is None:
            return self._scan_diag(pair, tf, f"score {score:+d} — no clear direction")
        ...
        if req_htf_of:
            j3 = bias.get("j3", 0)
            if (direction == "LONG" and j3 != 1) or (direction == "SHORT" and j3 != -1):
                return self._scan_diag(pair, tf, f"J3 (HTF orderflow) not aligned for {direction}")
        if req_dol:
            j2 = bias.get("j2", 0)
            if (direction == "LONG" and j2 != 1) or (direction == "SHORT" and j2 != -1):
                return self._scan_diag(pair, tf, f"J2 (DOL) not aligned for {direction}")

        # Skip Asian Session (00:00-08:00 Brusselse tijd) — laagvolume, choppy
        if skip_asian:
            now_b = now_brussels()
            if 0 <= now_b.hour < 8:
                return self._scan_diag(pair, tf, "asian session (00-08h Brussels) — skipped")
        ...
        if use_kz:
            ...
            if not in_kz:
                return self._scan_diag(pair, tf, f"outside killzones (now {h}h — London 9-12, NY 14-17)")
        ...
        if use_tr:
            ...
            if m2 > 0:
                ...
                if trend != 0 and trend != (1 if direction=="LONG" else -1):
                    return self._scan_diag(pair, tf, f"trend filter — trend against {direction}")
        ...
        if skip_spike_candle and n >= 15:
            ...
            if atr_val > 0 and last_range > 2.0 * atr_val:
                pip_size = PIP.get(pair, 0.0001)
                return self._scan_diag(pair, tf, f"spike protection — candle range {last_range/pip_size:.1f}p > 2×ATR")

        # FVG selection mode: "nearest" (dichtstbijzijnde, huidig) of "first" (oudste van cluster)
        fvg_select = cfg.get("fvg_select", "nearest")
        fvg = None
        _valid_fvgs = []  # for "first" mode
        _rej_fresh = 0; _rej_mit = 0; _rej_eq = 0; _found_any = 0
        for fi in range(n-1, max(n-20, 2), -1):
            ...
            _found_any += 1
            # Bars-since-formation check (wait for retest)
            bars_since = (n - 1) - f["formed_at"]
            if bars_since < min_bars_after_fvg:
                _rej_fresh += 1
                continue  # FVG too fresh — wait for retest from calm context
            # Mitigation check
            mitigated = False
            ...
            if use_eq:
                ...
                if (direction=="LONG" and mid >= eq_f) or (direction=="SHORT" and mid <= eq_f):
                    _rej_eq += 1
                    continue
            if fvg_select == "first":
                _valid_fvgs.append(f)  # collect all valid, pick oldest later
            else:
                fvg = f  # nearest: take first match and stop
                break

        # In "first" mode: pick the oldest valid FVG (closest to impulse origin)
        if fvg_select == "first" and _valid_fvgs:
            fvg = min(_valid_fvgs, key=lambda x: x["formed_at"])

        if fvg is None:
            if _found_any == 0:
                return self._scan_diag(pair, tf, f"{direction} bias OK but no {('bull' if direction=='LONG' else 'bear')} FVG w/ displacement in last 20 bars")
            return self._scan_diag(pair, tf, f"{direction} bias OK, {_found_any} FVG(s) rejected ({_rej_fresh} too fresh, {_rej_mit} mitigated, {_rej_eq} wrong EQ side)")
        ...
        if use_ob:
            for oi in range(fvg["formed_at"], max(fvg["formed_at"]-20, 1), -1):
                ...
                if o and o["type"] == ("bull" if direction=="LONG" else "bear"):
                    ob = o; break
            if ob is None:
                return self._scan_diag(pair, tf, f"FVG valid but no orderblock found (OB filter on)")
        ...
        if use_sw:
            ...
            if sweep is None:
                return self._scan_diag(pair, tf, "FVG valid but no liquidity sweep (sweep filter on)")
        ...
        if not in_fvg:
            return self._scan_diag(pair, tf, f"{direction} FVG valid [{fvg['bottom']:.5f}-{fvg['top']:.5f}] but price {current_price:.5f} outside zone — waiting for retrace")

        # HTF Bias filter — check hogere TF richting
        if use_htf:
            htf_result = check_htf_bias(pair, tf, direction)
            if htf_result is None:
                return self._scan_diag(pair, tf, "HTF bias — no data to verify, skipped for safety")
            if not htf_result["valid"]:
                return self._scan_diag(pair, tf, f"HTF bias not aligned for {direction}")
        ...
        if use_smt:
            ...
            if smt_result is None:
                return self._scan_diag(pair, tf, "SMT — DXY data unavailable, skipped")
            if not smt_result["valid"]:
                return self._scan_diag(pair, tf, f"SMT divergence not valid for {direction}")
        ...
        sl_pips_calc = 0
        if auto_sltp:
            # SL mode + caps from config (all default to current behaviour when unset)
            live_sl_mode     = cfg.get("sl_mode", "structural")   # "structural" or "atr"
            live_atr_mult    = float(cfg.get("atr_mult", 1.5))
            live_max_sl_pips = float(cfg.get("max_sl_pips", 0))    # 0 = no cap
            live_min_sl_pips = float(cfg.get("min_sl_pips", 0))    # 0 = no minimum
            live_min_sl_mode = cfg.get("min_sl_mode", "off")       # off/skip/stretch
            auto_sl, auto_tp, sl_pips_calc, sl_method = compute_sl_tp(
                df, fvg, direction, entry, pair, rr=rr,
                sl_mode=live_sl_mode, atr_mult_override=live_atr_mult,
                max_sl_pips=live_max_sl_pips,
                min_sl_pips=live_min_sl_pips, min_sl_mode=live_min_sl_mode)
            if auto_sl is None:
                # Risk te groot/klein OF SL buiten min/max grens → trade overslaan
                if sl_method == "sl_too_far":
                    self.log("INFO", f"Skip {pair} {tf}: SL further than max {live_max_sl_pips}p cap")
                elif sl_method == "sl_too_close":
                    self.log("INFO", f"Skip {pair} {tf}: SL closer than min {live_min_sl_pips}p (skip mode)")
                else:
        ...
        if use_dynamic_lot and risk_pct > 0 and auto_sltp and sl_pips_calc > 0:
            ...
            if balance:
                ...
                pip_per_std_lot = 9.20 if pair in ("EURUSD", "GBPUSD") else 0.92
        ...
        # ── FIXED-EURO SIZING (max loss per trade in €) ──
        # Alternative to percentage-based dynamic lot: size so a full-SL loss ≈ max_loss_eur.
        # Independent of account balance. Off by default (max_loss_eur = 0).
        max_loss_eur = float(cfg.get("max_loss_eur", 0))
        if max_loss_eur > 0 and auto_sltp and sl_pips_calc > 0 and not use_dynamic_lot:
            pip_per_std_lot = 9.20 if pair in ("EURUSD", "GBPUSD") else 0.92
            # lot in GAMAN units (100 = 1.0 std lot)
            euro_lot_gaman = (max_loss_eur / (sl_pips_calc * pip_per_std_lot)) * 100
            euro_lot_gaman = max(1, round(euro_lot_gaman))
            old_lot = lotsize
            lotsize = euro_lot_gaman
            self.log("INFO", f"Fixed-€ lot: {old_lot} → {lotsize} (max loss €{max_loss_eur}, SL {sl_pips_calc}p)")
        ...
        # ── Rich context capture at OPEN (for later analysis/ML) ──
        try:
            trade["context"] = capture_trade_context(
                df, pair, tf, direction, entry, fvg, bias, score,
                sl_pips_calc, engine_ref=self)
            # MAE/MFE trackers (updated during monitoring)
            trade["mae_pips"] = 0.0   # max adverse excursion (worst drawdown in trade)
            trade["mfe_pips"] = 0.0   # max favorable excursion (best profit in trade)
        except Exception as e:
            self.log("WARN", f"Context capture failed: {e}")

        with self.lock:
            self.open_trades.append(trade)
            self.recent_entries[f"{pair}_{tf}"] = time.time()
        self.scan_status[f"{pair}_{tf}"] = "TRADE OPENED"
    ...
    def _monitor_open_trades(self):
        ...
        for trade in trades_to_check:
            ...
            with self.lock:
                for t in self.open_trades:
                    if t["id"] == trade["id"]:
                        ...
                        # MAE/MFE tracking (max adverse / favorable excursion in pips)
                        if pips < t.get("mae_pips", 0.0):
                            t["mae_pips"] = round(pips, 1)  # most negative
                        if pips > t.get("mfe_pips", 0.0):
                            t["mfe_pips"] = round(pips, 1)  # most positive
            ...
            # ── BREAK-EVEN: verplaats SL naar entry bij winst-trigger ──
            # be_trigger = X (bv 1.0 = bij 1R winst). Config-gestuurd, default 0 (uit).
            # Verplaatst SL fysiek via MT5 modify_order, 1× per trade.
            be_trigger = float(self.config.get("be_trigger", 0) or 0)
            if be_trigger > 0 and sl is not None and not trade.get("be_moved", False):
                sl_pips_orig = trade.get("sl_pips", 0) or 0
                if sl_pips_orig > 0:
                    # Winst in R = huidige pips winst / originele SL afstand
                    r_now = pips / sl_pips_orig
                    if r_now >= be_trigger:
                        # Verplaats SL naar entry (break-even)
                        new_sl = entry
                        moved_ok = True
                        if mt5_bridge.enabled and mt5_bridge.connected:
                            moved_ok = mt5_bridge.modify_order(trade["id"], new_sl=new_sl, new_tp=tp)
                        if moved_ok:
                            with self.lock:
                                for t in self.open_trades:
                                    if t["id"] == trade["id"]:
                                        t["sl"] = new_sl
                                        t["be_moved"] = True
                            sl = new_sl  # gebruik nieuwe SL meteen in de hit-check hieronder
                            self.log("INFO", f"Break-even: trade #{trade['id']} {pair} SL → entry ({r_now:.2f}R bereikt)")
                            self._save_state()
            ...
            if hit:
                ...
                # ── Outcome labels for analysis ──
                sl_pips_val = trade.get("sl_pips", 0) or 1
                r_multiple = round(final_pips / sl_pips_val, 2) if sl_pips_val else 0
                if abs(final_pnl) < 0.01:
                    outcome_label = "BE"
                elif final_pnl > 0:
                    outcome_label = "win"
                else:
                ...
                closed = {
                    **trade,
                    ...
                    # Rich outcome data
                    "exit_reason":    hit,
                    "outcome_label":  outcome_label,   # win / loss / BE
                    "r_multiple":     r_multiple,      # profit in R (relative to SL distance)
                    "mae_pips":       trade.get("mae_pips", 0.0),
                    "mfe_pips":       trade.get("mfe_pips", 0.0),
                    "duration_min":   round((int(now_brussels().timestamp()) - trade.get("opened_ts", 0)) / 60, 0),
                }
                with self.lock:
...
SYMBOLS   = {"EURUSD":"EURUSD=X","GBPUSD":"GBPUSD=X","XAUUSD":"GC=F","DXY":"DX-Y.NYB"}
TF_YF     = {"15M":"15m","1H":"1h","4H":"1h"}
TF_PERIOD = {"15M":"7d","1H":"730d","4H":"730d"}
PIP       = {"EURUSD":0.0001,"GBPUSD":0.0001,"XAUUSD":0.10}
PIP_EUR   = {"EURUSD":0.10,"GBPUSD":0.10,"XAUUSD":0.92}
...
def compute_sl_tp(df, fvg, direction, entry, pair, rr=2.0, sl_mode="structural", atr_mult_override=None, max_sl_pips=0, min_sl_pips=0, min_sl_mode="off"):
    """
    ...
    # Optional override of ATR multiplier (from backtester ATR-SL mode)
    if atr_mult_override is not None and atr_mult_override > 0:
        atr_multiplier = float(atr_mult_override)
    ...
    if direction == "LONG":
        if sl_mode == "atr" and atr > 0:
            # ATR-SL mode: SL purely based on volatility, ignore distant swing/OB levels
            sl = entry - atr * atr_multiplier
            sl_method = "atr_forced"
        else:
    ...
    # Optional max SL cap: if SL is further than max_sl_pips, reject the trade.
    # Guards against extreme SL distances (e.g. distant OB during high volatility).
    if max_sl_pips and max_sl_pips > 0:
        sl_pips_check = risk / pip_v
        if sl_pips_check > max_sl_pips:
            return None, None, 0, "sl_too_far"

    # Optional MINIMUM SL distance — protects against tiny trades that slippage
    # would eat alive (e.g. a 6-pip SL where 3 pips slippage leaves 3 pips buffer).
    # min_sl_mode: "off" (no minimum), "skip" (reject trade), "stretch" (widen SL to min).
    # When stretching, risk grows and TP follows automatically (RR stays constant).
    if min_sl_pips and min_sl_pips > 0 and min_sl_mode != "off":
        sl_pips_now = risk / pip_v
        if sl_pips_now < min_sl_pips:
            if min_sl_mode == "skip":
                return None, None, 0, "sl_too_close"
            elif min_sl_mode == "stretch":
                # Widen SL to the minimum distance; risk grows, TP follows via risk*rr
                risk = min_sl_pips * pip_v
                if direction == "LONG":
                    sl = entry - risk
                else:
                    sl = entry + risk
                sl_method = sl_method + "+stretched"
...
def run_backtest(pair, tf, start, end, capital, lotsize, rr, use_ob, use_trend, use_eq, min_score, use_session=False, use_sweep=False, use_htf_bias=False, use_smt=False, skip_asian=False, skip_holidays=False, require_htf_orderflow=False, require_dol=False, be_trigger=0.0, spread_pips=0.0, slippage_pips=0.0, lotsize_eur=None, lotsize_gbp=None, lotsize_xau=None, max_daily_loss=0, max_trades=0, max_risk_pct=0, **kwargs):
    ...
    for i in range(20, n - 2):
        ...
        # fvg_select: "nearest" (dichtstbijzijnde/meest recent, huidig gedrag)
        #          of "first" (eerste/oudste van het cluster — dichter bij impuls-oorsprong)
        fvg_select = kwargs.get("fvg_select", "nearest")
        fvg = None
        valid_fvgs = []  # for "first" mode: collect all valid, then pick oldest
        for fi in range(i-1, max(i-30, 2), -1):
            ...
            if fvg_select == "first":
                valid_fvgs.append(f)  # collect, decide later
            else:
                fvg = f  # nearest: take first match and stop
                break

        # In "first" mode: pick the oldest valid FVG (closest to impulse origin)
        if fvg_select == "first" and valid_fvgs:
            fvg = min(valid_fvgs, key=lambda x: x["formed_at"])
        ...
        # LIMIT-ORDER SIMULATION MODE
        # When limit_mode is on, a trade only fills if price touches the FVG midpoint
        # within `limit_expiry_bars` candles after FVG formation (simulating a pending
        # limit order that expires). Also records fill timing for diagnostics.
        limit_mode = bool(kwargs.get("limit_mode", False))
        limit_expiry_bars = int(kwargs.get("limit_expiry_bars", 3))
        limit_fill_offset = None  # how many bars after FVG formation the fill occurred

        if limit_mode:
            fvg_mid = (fvg["top"] + fvg["bottom"]) / 2
            # Scan window: from formation+1 up to formation+expiry (or current bar i)
            scan_end = min(fvg["formed_at"] + 1 + limit_expiry_bars, i + 2, n - 1)
            for j in range(fvg["formed_at"] + 1, scan_end):
                c_high = float(df.iloc[j]["high"])
                c_low  = float(df.iloc[j]["low"])
                # Limit order fills only if price actually reaches the exact midpoint
                if direction == "LONG":
                    if c_low <= fvg_mid:  # price dipped to/below midpoint → limit filled
                        entry_price = fvg_mid
                        entry_bar   = j
                        limit_fill_offset = j - fvg["formed_at"]
                        break
                    elif c_low < fvg["bottom"]:
                        break  # mitigated before fill
                else:
                    if c_high >= fvg_mid:  # price rose to/above midpoint → limit filled
                        entry_price = fvg_mid
                        entry_bar   = j
                        limit_fill_offset = j - fvg["formed_at"]
                        break
                        ...
                        break  # mitigated before fill
            # If not filled within expiry window → order expires, no trade
        else:
            # MARKET MODE — IDENTIEK aan live engine: entry alleen als een candle
            # SLUIT binnen de FVG-zone (live checkt df.iloc[-1]["close"] in zone).
            # Geen intra-candle low/high aanrakingen meer — die telden trades die
            # de live engine nooit zou nemen (candle raakt zone maar sluit erbuiten).
            for j in range(fvg["formed_at"]+1, min(i+2, n-1)):
                c_close = float(df.iloc[j]["close"])
                c_low   = float(df.iloc[j]["low"])
                c_high  = float(df.iloc[j]["high"])

                if direction == "LONG":
                    if fvg["bottom"] <= c_close <= fvg["top"]:   # sluit binnen zone → entry
                        entry_price = (fvg["top"] + fvg["bottom"]) / 2
                        entry_bar   = j; break
                    elif c_low < fvg["bottom"]: break  # zone doorbroken → gemitigeerd
                else:
                    if fvg["bottom"] <= c_close <= fvg["top"]:   # sluit binnen zone → entry
                        entry_price = (fvg["top"] + fvg["bottom"]) / 2
                        entry_bar   = j; break
                    elif c_high > fvg["top"]: break  # zone doorbroken → gemitigeerd
        ...
        # ── Punt 1 — SL berekening via compute_sl_tp (IDENTIEK aan live engine) ──
        # Gebruikt exact dezelfde functie als de live engine voor 1-op-1 consistentie.
        # df-slice tot entry_bar+1 voorkomt toekomst-lek: swing-detectie kan niet
        # vooruitkijken voorbij het instap-moment.
        bt_sl_mode      = kwargs.get("sl_mode", "structural")
        bt_atr_mult     = float(kwargs.get("atr_mult", 1.5))
        bt_max_sl_pips  = float(kwargs.get("max_sl_pips", 0))

        df_slice = df.iloc[:entry_bar+1]
        bt_min_sl_pips = float(kwargs.get("min_sl_pips", 0))
        bt_min_sl_mode = kwargs.get("min_sl_mode", "off")
        sl, tp, sl_pips, sl_method = compute_sl_tp(
            df_slice, fvg, direction, raw_entry, pair, rr=rr,
            sl_mode=bt_sl_mode, atr_mult_override=bt_atr_mult,
            max_sl_pips=bt_max_sl_pips,
            min_sl_pips=bt_min_sl_pips, min_sl_mode=bt_min_sl_mode)
        if sl is None:
            continue  # invalid risk / SL te ver (max cap) / SL te dichtbij (skip mode) → skip trade
        ...
        # ── Money-based position sizing (max loss per trade) — IDENTIEK aan live ──
        # Live engine gebruikt: lot_gaman = (max_loss / (sl_pips * pip_per_std_lot)) * 100
        # met pip_per_std_lot = 9.20 (EUR/GBP) of 0.92 (XAU). We gebruiken exact dezelfde
        # formule zodat backtest-lotsize 1-op-1 overeenkomt met wat live zou openen.
        eff_lotsize = lotsize
        max_loss_eur = float(kwargs.get("max_loss_eur", 0))
        if max_loss_eur > 0 and sl_pips > 0:
            pip_per_std_lot = 9.20 if pair in ("EURUSD", "GBPUSD") else 0.92
            eff_lotsize = (max_loss_eur / (sl_pips * pip_per_std_lot)) * 100
            eff_lotsize = max(1, round(eff_lotsize))  # GAMAN lot units, min 1

        pnl  = round(pips * pip_e * eff_lotsize, 2)
        ...
        trades.append({
            ...
            "limit_fill_offset": limit_fill_offset,  # bars after FVG formation (None if market mode)
            "eff_lotsize": eff_lotsize,  # actual lot used (varies if money-based sizing on)
        })
    ...
    if not trades:
        ...
        # Limit-order fill timing diagnostics (only meaningful in limit_mode)
        if kwargs.get("limit_mode", False):
            offsets = [t["limit_fill_offset"] for t in trades if t.get("limit_fill_offset") is not None]
            if offsets:
                from collections import Counter
                dist = Counter(offsets)
                stats["limit_fill_timing"] = {str(k): dist[k] for k in sorted(dist.keys())}
                stats["limit_avg_fill_bars"] = round(sum(offsets)/len(offsets), 2)
                stats["limit_filled_count"] = len(offsets)
...
def api_export_csv():
    """Export all closed trades with full context as CSV for analysis."""
    import csv, io
    trades = engine.closed_trades
    if not trades:
        return Response("no closed trades yet", mimetype="text/plain")
    rows = []
    for t in trades:
        row = {k: v for k, v in t.items() if k != "context"}
        ctx = t.get("context", {})
    ...
    return Response(
        out.getvalue(),
        mimetype="text/csv",
        headers={"Content-Disposition": "attachment; filename=gaman_trades.csv"}
    )

@app.route("/api/trades/export/json")
def api_export_json():
    """Export all closed trades with full context as JSON."""
    import json as _json
    data = _json.dumps(engine.closed_trades, indent=2, default=str)
    return Response(
        data,
        mimetype="application/json",
        headers={"Content-Disposition": "attachment; filename=gaman_trades.json"}
    )

@app.route("/api/trades/context/<int:trade_id>")
def api_trade_context(trade_id):
    """Return full context for one trade (detail view)."""
    for t in engine.closed_trades + engine.open_trades:
        if t.get("id") == trade_id:
            return jsonify({"ok": True, "trade": t})
    return jsonify({"ok": False, "error": "trade not found"}), 404

@app.route("/api/mt5/health", methods=["POST"])
def api_mt5_health():
    """Full MT5 bridge health check — non-destructive, no orders placed.
    Verifies each step of the communication path with clear results.
    """
    from pathlib import Path
    import os
    checks = []
    ...
    # Check 1: files dir exists + writable
    files_dir = Path(mt5_bridge._files_dir)
    exists = files_dir.exists()
    writable = False
    try:
        test_file = files_dir / ".gaman_write_test"
        ...
        writable = False
    checks.append({
        "step":   "1. Files directory",
        "ok":     exists and writable,
        "detail": f"{files_dir}" + (" ✓ writable" if writable else " ✗ not writable" if exists else " ✗ does not exist"),
    })
    if not (exists and writable): ok_all = False

    # Check 2: heartbeat file exists + recent
    hb_path = files_dir / MT5_HEARTBEAT_FILE
    if not hb_path.exists():
        checks.append({"step":"2. EA heartbeat file","ok":False,"detail":f"{MT5_HEARTBEAT_FILE} not found — EA has never written yet"})
        ok_all = False
    else:
        try:
            age = time.time() - hb_path.stat().st_mtime
            fresh = age < 30
            checks.append({
                "step":   "2. EA heartbeat file",
                "ok":     fresh,
                "detail": f"exists, last updated {int(age)}s ago" + (" ✓" if fresh else " ✗ STALE — EA may have stopped"),
            })
            if not fresh: ok_all = False
        except Exception as e:
            checks.append({"step":"2. EA heartbeat file","ok":False,"detail":f"read error: {e}"})
            ok_all = False

    # Check 3: heartbeat content parses + has account info
    account_info = mt5_bridge.account_info()
    if account_info:
        checks.append({
            "step":   "3. Account info",
            "ok":     True,
            "detail": f"account {account_info.get('login')} · balance €{account_info.get('balance',0):.2f} · equity €{account_info.get('equity',0):.2f} · open positions: {account_info.get('positions',0)}",
        })
    else:
        checks.append({"step":"3. Account info","ok":False,"detail":"heartbeat does not contain valid account data"})
        ok_all = False

    # Check 4: EA responds to ping via round-trip
    # We write a PING order and see if EA writes a result within 5s.
    # Uses action "PING" which the EA should ignore for real orders but still write a result.
    # Fallback: if EA doesn't support PING, we detect by result-file age change after a NOOP write.
    ping_id = "ping-" + str(int(time.time()))
    ping_cmd = {
        "id":     ping_id,
        "action": "PING",           # EA sees this, should reply success:false with "unknown action"
        "symbol": "",
        "side":   "",
        "volume": 0.0,
        "sl":     0.0,
    ...
    if mt5_bridge.connected:
        start_ts = time.time()
        wrote = mt5_bridge._write_order(ping_cmd)
        if wrote:
            result = mt5_bridge._read_result(ping_id, timeout=5.0)
            elapsed = int((time.time() - start_ts) * 1000)
            if result is not None:
                round_trip_ok = True
                round_trip_ms = elapsed
                round_trip_detail = f"EA responded in {elapsed}ms ✓"
            else:
                round_trip_detail = "no response within 5s — EA reads files but is not writing back (check EA log in MT5 → Experts tab)"
        else:
            round_trip_detail = "could not write order file — permissions?"
    else:
        round_trip_detail = "skipped — heartbeat offline"
    checks.append({
        "step":   "4. Round-trip test",
        "ok":     round_trip_ok,
        "detail": round_trip_detail,
    })
    if not round_trip_ok: ok_all = False

    # Check 5: toggle status
    checks.append({
        "step":   "5. Execution toggle",
        "ok":     mt5_bridge.enabled,
        "detail": "ON — orders will be sent to MT5" if mt5_bridge.enabled else "OFF — GAMAN paper trades only, MT5 will NOT receive orders",
    })
    # Not counting toggle-off as fail, just informational

    engine.log("INFO", f"[MT5] Health check: {'ALL OK' if ok_all else 'ISSUES FOUND'}")
    return jsonify({
        "ok":       ok_all,
        "checks":   checks,
        "summary":  "All systems operational" if ok_all else "Issues detected — review checks below",
    })

@app.route("/api/mt5/test_trade", methods=["POST"])
def api_mt5_test_trade():
    """Send a REAL 0.01 lot market order to MT5, wait 10 seconds, then close.
    Purpose: end-to-end verification of the signal pipeline on live/demo account.
    Bypasses GAMAN engine state — order is NOT recorded in dashboard trade history.
    Expected cost: ~€0.20-0.50 loss due to spread + slippage on 0.01 lot.
    """
    body = request.json or {}
    side = str(body.get("side","BUY")).upper()
    symbol = str(body.get("symbol","EURUSD")).upper()
    hold_seconds = int(body.get("hold_seconds", 10))
    if side not in ("BUY","SELL"):
        return jsonify({"ok": False, "error": "side must be BUY or SELL"}), 400
    if symbol not in ("EURUSD","GBPUSD","XAUUSD"):
        return jsonify({"ok": False, "error": "symbol must be EURUSD, GBPUSD or XAUUSD"}), 400
    if not mt5_bridge.connected:
        return jsonify({"ok": False, "error": "MT5 EA offline — cannot send order"}), 400

    # Force enable temporarily if toggle is off, restore afterwards
    was_enabled = mt5_bridge.enabled
    mt5_bridge.enabled = True

    import uuid as _uuid
    open_id  = "test-" + _uuid.uuid4().hex[:8]
    close_id = "clos-" + _uuid.uuid4().hex[:8]
    result_log = {
        "symbol":       symbol,
        "side":         side,
    ...
    try:
        engine.log("INFO", f"[MT5-TEST] Sending test trade: {side} {symbol} 0.01 lot, will auto-close in {hold_seconds}s")
        ...
        if not mt5_bridge._write_order(open_cmd):
            result_log["phases"].append({"phase":"OPEN","ok":False,"detail":"failed to write order file"})
            return jsonify({"ok": False, "error": "failed to write order file", "result": result_log}), 500

        open_result = mt5_bridge._read_result(open_id, timeout=8.0)
        open_elapsed = int((time.time() - t0) * 1000)
        if open_result is None:
            result_log["phases"].append({"phase":"OPEN","ok":False,"detail":f"no response within 8s (elapsed {open_elapsed}ms)"})
            return jsonify({"ok": False, "error": "OPEN timeout", "result": result_log}), 500
        if not open_result.get("success"):
            err = open_result.get("error","unknown")
            result_log["phases"].append({"phase":"OPEN","ok":False,"detail":f"EA rejected: {err}"})
            return jsonify({"ok": False, "error": f"OPEN failed: {err}", "result": result_log}), 500

        ticket = int(open_result.get("ticket", 0))
        entry_price = float(open_result.get("entry_price", 0))
        result_log["phases"].append({
            "phase":  "OPEN",
            "ok":     True,
            "detail": f"ticket #{ticket} @ {entry_price:.5f} · {open_elapsed}ms round-trip",
        })
        engine.log("INFO", f"[MT5-TEST] OPEN OK: ticket={ticket} entry={entry_price:.5f} ({open_elapsed}ms)")
        ...
        result_log["phases"].append({
            "phase":  "HOLD",
            "ok":     True,
            "detail": f"held for {hold_seconds}s to observe execution",
        })
        ...
        if not mt5_bridge._write_order(close_cmd):
            result_log["phases"].append({"phase":"CLOSE","ok":False,"detail":f"failed to write close file — MANUAL CLOSE REQUIRED for ticket {ticket}"})
            return jsonify({"ok": False, "error": "close write failed", "result": result_log, "manual_close_ticket": ticket}), 500

        close_result = mt5_bridge._read_result(close_id, timeout=8.0)
        close_elapsed = int((time.time() - t1) * 1000)
        if close_result is None:
            result_log["phases"].append({"phase":"CLOSE","ok":False,"detail":f"no close response within 8s — MANUAL CLOSE REQUIRED for ticket {ticket}"})
            return jsonify({"ok": False, "error": "CLOSE timeout — check MT5 and close manually", "result": result_log, "manual_close_ticket": ticket}), 500
        if not close_result.get("success"):
            err = close_result.get("error","unknown")
            result_log["phases"].append({"phase":"CLOSE","ok":False,"detail":f"EA rejected close: {err} — MANUAL CLOSE REQUIRED for ticket {ticket}"})
            return jsonify({"ok": False, "error": f"CLOSE failed: {err}", "result": result_log, "manual_close_ticket": ticket}), 500

        result_log["phases"].append({
            "phase":  "CLOSE",
            "ok":     True,
            "detail": f"ticket #{ticket} closed · {close_elapsed}ms round-trip",
        })
        result_log["summary"] = f"Full round-trip OK · ticket {ticket} · {side} {symbol} 0.01 lot @ {entry_price:.5f} · held {hold_seconds}s"
        engine.log("INFO", f"[MT5-TEST] CLOSE OK: ticket={ticket} ({close_elapsed}ms)")
        return jsonify({"ok": True, "result": result_log})
    except Exception as e:
        engine.log("ERROR", f"[MT5-TEST] Exception: {e}")
        return jsonify({"ok": False, "error": str(e), "result": result_log}), 500
    finally:
        mt5_bridge.enabled = was_enabled
...
def api_backtest():
    ...
    kwargs = dict(
        ...
        lotsize_gbp  = float(d.get("lotsize_gbp", d.get("lotsize_eur", d.get("lotsize",1)))),
        ...
        limit_mode        = bool(d.get("limit_mode", False)),
        limit_expiry_bars = int(d.get("limit_expiry_bars", 3)),
        fvg_select        = d.get("fvg_select", "nearest"),
        sl_mode           = d.get("sl_mode", "structural"),
        atr_mult          = float(d.get("atr_mult", 1.5)),
        max_sl_pips       = float(d.get("max_sl_pips", 0)),
        min_sl_pips       = float(d.get("min_sl_pips", 0)),
        min_sl_mode       = d.get("min_sl_mode", "off"),
        max_loss_eur      = float(d.get("max_loss_eur", 0)),
    )
    ...
    # Determine which pairs and timeframes to run
    # New: accept "pairs" array (e.g. ["EURUSD","GBPUSD"]) with fallback to legacy "pair" string
    cfg_pairs = d.get("pairs")
    if cfg_pairs and isinstance(cfg_pairs, list) and len(cfg_pairs) > 0:
        pairs = [p for p in cfg_pairs if p in PIP]  # filter to valid pairs
        if not pairs: pairs = ["EURUSD"]
    else:
        # Legacy: pair="BOTH" means EUR+XAU (backward compat)
...
def api_optimize():
    ...
    lotsize_gbp  = float(d.get("lotsize_gbp", lotsize_eur))
    ...
    # Minimum trades threshold — combos below this are disqualified BEFORE ranking.
    # Guards against statistically thin "winners" (e.g. 30 trades with lucky 80% WR).
    # Applied to BOTH in-sample and out-of-sample counts.
    min_trades_is  = int(d.get("min_trades", 200))  # default 200 as user requested
    # OOS is 30% of data → scale threshold proportionally (30/70 of in-sample min)
    min_trades_oos = max(int(min_trades_is * 0.3), 20)
                        ...
                        for use_eq in eq_opts:
                            for use_kz in kz_opts:
                                for be in be_opts:
                                    ...
                                    if len(is_trades) < min_trades_is:
                                        continue  # disqualified: too few in-sample trades
                                    ...
                                    # Flag whether OOS has enough trades to be trustworthy
                                    oos_sufficient = oos_stats["total"] >= min_trades_oos
                                    ...
                                    results.append({
                                        ...
                                        # Score: OOS weighted heavier; heavy penalty if OOS too thin
                                        "score_val": (is_stats["winrate"] * 0.4 +
                                                      oos_stats["winrate"] * 0.6 +
                                                      (1 if oos_stats["total_pnl"] > 0 else -10) +
                                                      (0 if oos_sufficient else -50)),  # thin OOS = big penalty
                                    })
...
<div id="app">
  ...
  <div id="left">
    ...
    <div class="card">
      <div class="chart-toolbar">
        <div class="btn-group" id="pg">
          ...
          <button class="toggle-btn" onclick="setPair('GBPUSD')">GBP/USD</button>
    ...
    <div class="card" id="live-engine-log-card">
      <div class="card-header">
        ...
        <div style="display:flex;gap:6px">
          ...
          <button class="csv-btn" onclick="exportLiveJSON()">↓ Trades JSON</button>
        </div>
      </div>
    ...
    <div class="card" id="bt-results" style="display:none">
      ...
      <div class="card-body">
        ...
        <div id="bt-limit-diag" style="display:none;margin:10px 0;padding:10px;border-radius:6px;background:rgba(96,165,250,.06);border:1px solid rgba(96,165,250,.2)"></div>
        <div class="tbl-wrap">
          <table>
  ...
  <div id="right">
      ...
      <div class="card-body">
        <div class="form-grid">
          <div class="form-group"><label>Pair</label>
            <div id="lt-pair-chips" style="display:flex;gap:6px;flex-wrap:wrap;margin-top:4px">
              <div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--glow2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.15);color:var(--glow2);user-select:none;transition:.15s;font-weight:600">EURUSD</div>
              <div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">GBPUSD</div>
              <div class="pair-chip" data-pair="XAUUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">XAUUSD</div>
            </div>
            <input type="hidden" id="lt-pair" value="EURUSD">
          </div>
          ...
          <div class="form-group"><label>Lot GBPUSD (micro)</label>
            <input type="number" id="lt-lot-gbp" value="1" min="1" step="1" title="Lotsize for GBP/USD trades">
          </div>
        ...
        <div class="toggle-section collapse-section collapsed" id="mt5-section">
          ...
          <button onclick="runMT5HealthCheck()" id="mt5-health-btn" style="margin-top:8px;width:100%;padding:8px;border:1px solid var(--border2);border-radius:5px;background:rgba(255,255,255,.05);color:var(--text);font-size:11px;font-family:'Inter',sans-serif;cursor:pointer;transition:.2s" onmouseover="this.style.background='rgba(255,255,255,.12)'" onmouseout="this.style.background='rgba(255,255,255,.05)'">
            🔍 Run Full MT5 Health Check
          </button>
          <div id="mt5-health-result" style="margin-top:8px;display:none;font-size:10px;font-family:'JetBrains Mono',monospace;line-height:1.5"></div>

          <!-- Live test trade — real 0.01 lot order → close after 10s -->
          <div style="margin-top:10px;padding:8px;border:1px dashed rgba(251,191,36,.4);border-radius:5px;background:rgba(251,191,36,.05)">
            <div style="font-size:10px;color:var(--amber);font-weight:600;margin-bottom:4px">⚠ Live Test Trade (REAL order)</div>
            <div style="font-size:9px;color:var(--text3);line-height:1.4;margin-bottom:6px">
              Sends real 0.01 lot market order to MT5, holds 10s, then auto-closes. Cost: ~€0.20-0.50 (spread + slippage). Use to verify end-to-end signal flow.
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:6px">
              <select id="mt5-test-symbol" style="padding:5px;font-size:10px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:3px">
                <option value="EURUSD">EURUSD</option>
                <option value="GBPUSD">GBPUSD</option>
                <option value="XAUUSD">XAUUSD</option>
              </select>
              <select id="mt5-test-side" style="padding:5px;font-size:10px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:3px">
                <option value="BUY">BUY</option>
                <option value="SELL">SELL</option>
              </select>
            </div>
            <button onclick="runMT5TestTrade()" id="mt5-test-btn" style="width:100%;padding:8px;border:1px solid var(--amber);border-radius:5px;background:rgba(251,191,36,.15);color:var(--amber);font-size:11px;font-weight:600;cursor:pointer;transition:.2s" onmouseover="this.style.background='rgba(251,191,36,.3)'" onmouseout="this.style.background='rgba(251,191,36,.15)'">
              📤 Send Test Trade (0.01 lot, auto-close in 10s)
            </button>
            <div id="mt5-test-result" style="margin-top:8px;display:none;font-size:10px;font-family:'JetBrains Mono',monospace;line-height:1.5"></div>
          </div>
        ...
        <div class="toggle-section collapse-section collapsed">
          ...
          <div class="form-group" style="margin-top:12px">
            <label>FVG selectie (bij meerdere FVG's)</label>
            <select id="lt-fvg-select" style="width:100%;padding:8px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
              <option value="nearest">Dichtstbijzijnde / meest recent (huidig)</option>
              <option value="first">Eerste van cluster (dichtst bij oorsprong)</option>
            </select>
          </div>

          <!-- SL MODE (live) -->
          <div class="form-group" style="margin-top:12px">
            <label>SL berekening</label>
            <select id="lt-sl-mode" onchange="document.getElementById('lt-atr-opts').style.display=this.value==='atr'?'block':'none'" style="width:100%;padding:8px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
              <option value="structural">Structureel (swing — huidig)</option>
              <option value="atr">ATR-gebaseerd (volatiliteit)</option>
            </select>
          </div>
          <div id="lt-atr-opts" style="display:none">
            <div class="form-group"><label>ATR multiplier</label><input type="number" id="lt-atr-mult" value="1.5" min="0.5" max="5" step="0.1"></div>
          </div>
          <div class="form-group">
            <label>Max SL afstand (pips, 0=uit)</label>
            <input type="number" id="lt-max-sl" value="0" min="0" max="200" step="5" title="Skip trades met SL verder dan dit">
          </div>
          <div class="form-group">
            <label>Min SL afstand (pips, 0=uit)</label>
            <input type="number" id="lt-min-sl" value="0" min="0" max="100" step="1" title="Beschermt tegen te krappe trades die slippage opeet">
          </div>
          <div class="form-group">
            <label>Bij te krappe SL</label>
            <select id="lt-min-sl-mode" style="width:100%;padding:8px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
              <option value="off">Uit</option>
              <option value="skip">Overslaan</option>
              <option value="stretch">Oprekken (TP volgt RR)</option>
            </select>
          </div>
          <div class="form-group">
            <label>Max verlies per trade (€, 0=uit)</label>
            <input type="number" id="lt-max-loss-trade" value="0" min="0" max="500" step="5" title="Geld-gebaseerde lotsize. Overschrijft vaste lot. Werkt niet samen met Dynamic Lot.">
          </div>
          <div style="font-size:9px;color:var(--amber);line-height:1.4;padding:6px 8px;background:rgba(251,191,36,.06);border-radius:6px;margin-top:4px">
            ⚠ Max verlies per trade laat je lotsize variëren per trade — dit vervangt je vaste exposure. Test eerst in de backtester.
          </div>
          <div class="form-group" style="margin-top:10px">
            <label>Break-even trigger (R, 0=uit)</label>
            <input type="number" id="lt-be-trigger" value="0" min="0" max="5" step="0.1" title="Verplaats SL naar entry als trade X×risk in winst staat. Bv 1.0 = bij 1R winst. Vereist Auto SL/TP + verbonden EA.">
          </div>
          <div style="font-size:9px;color:var(--amber);line-height:1.4;padding:6px 8px;background:rgba(251,191,36,.06);border-radius:6px;margin-top:4px">
            ⚠ Break-even verplaatst je SL live via de EA. Test eerst op demo voordat je dit live vertrouwt.
          </div>
          </div>
        </div>
    ...
    <div class="card" id="bt-form" style="display:none">
      ...
      <div class="card-body">
        <div class="form-grid">
          <div class="form-group"><label>Pairs (select 1 or more)</label>
            <div id="bt-pair-chips" style="display:flex;gap:6px;flex-wrap:wrap;margin-top:4px">
              <div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this); updateBtPairFields()" style="padding:6px 12px;border:1px solid var(--glow2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.15);color:var(--glow2);user-select:none;transition:.15s;font-weight:600">EURUSD</div>
              <div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this); updateBtPairFields()" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">GBPUSD</div>
              <div class="pair-chip" data-pair="XAUUSD" onclick="togglePairChip(this); updateBtPairFields()" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">XAUUSD</div>
            </div>
            <input type="hidden" id="bt-pair" value="EURUSD">
          </div>
          ...
          <div class="form-group"><label>Lot GBPUSD (micro)</label><input type="number" id="bt-lot-gbp" value="1" min="1" step="1"></div>
          ...
          <div style="margin-top:12px;padding:10px;border:1px dashed rgba(96,165,250,.4);border-radius:6px;background:rgba(96,165,250,.05)">
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:12px">
              <input type="checkbox" id="bt-limit-mode" onchange="document.getElementById('bt-limit-opts').style.display=this.checked?'block':'none'">
              <span style="font-weight:600;color:var(--glow2)">Limit Order Mode (simulatie)</span>
            </label>
            <div style="font-size:9px;color:var(--text3);margin-top:4px;line-height:1.4">
              Trade vult alleen als prijs de FVG-midpoint exact raakt binnen X candles na FVG-vorming (simuleert een pending limit order die vervalt). Meet fill-timing.
            </div>
            <div id="bt-limit-opts" style="display:none;margin-top:8px">
              <div class="form-group"><label>Limit expiry (candles)</label><input type="number" id="bt-limit-expiry" value="3" min="1" max="20" step="1" title="Order vervalt na dit aantal candles"></div>
            </div>
          </div>

          <!-- FVG SELECTION -->
          <div style="margin-top:12px;padding:10px;border:1px dashed rgba(167,139,250,.4);border-radius:6px;background:rgba(167,139,250,.05)">
            <label style="font-size:12px;font-weight:600;color:var(--purple)">FVG selectie (bij meerdere FVG's)</label>
            <select id="bt-fvg-select" style="width:100%;margin-top:6px;padding:6px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
              <option value="nearest">Dichtstbijzijnde / meest recent (huidig)</option>
              <option value="first">Eerste van cluster (dichtst bij impuls-oorsprong)</option>
            </select>
            <div style="font-size:9px;color:var(--text3);margin-top:4px;line-height:1.4">
              Bij 3 FVG's na elkaar: "dichtstbijzijnde" pakt de meest extreme, "eerste" pakt die het dichtst bij het orderblock ligt (ICT-theorie).
            </div>
          </div>

          <!-- SL MODE -->
          <div style="margin-top:12px;padding:10px;border:1px dashed rgba(52,211,153,.4);border-radius:6px;background:rgba(52,211,153,.05)">
            <label style="font-size:12px;font-weight:600;color:var(--green)">SL berekening</label>
            <select id="bt-sl-mode" onchange="document.getElementById('bt-atr-opts').style.display=this.value==='atr'?'block':'none'" style="width:100%;margin-top:6px;padding:6px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
              <option value="structural">Structureel (swing low/high — huidig)</option>
              <option value="atr">ATR-gebaseerd (volatiliteit — vermijdt verre SL)</option>
            </select>
            <div id="bt-atr-opts" style="display:none;margin-top:8px">
              <div class="form-group"><label>ATR multiplier</label><input type="number" id="bt-atr-mult" value="1.5" min="0.5" max="5" step="0.1" title="SL = entry ± (multiplier × ATR). 1.5 is standaard."></div>
            </div>
            <div class="form-group" style="margin-top:8px"><label>Max SL afstand (pips, 0=uit)</label><input type="number" id="bt-max-sl" value="0" min="0" max="200" step="5" title="Skip trades met SL verder dan dit. Voorkomt extreme SL bij hoge volatiliteit."></div>
            <div class="form-group" style="margin-top:8px">
              <label>Min SL afstand (pips, 0=uit)</label>
              <input type="number" id="bt-min-sl" value="0" min="0" max="100" step="1" title="Beschermt tegen te krappe trades die slippage opeet.">
            </div>
            <div class="form-group" style="margin-top:6px">
              <label>Bij te krappe SL</label>
              <select id="bt-min-sl-mode" style="width:100%;padding:6px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
                <option value="off">Uit</option>
                <option value="skip">Overslaan (trade niet nemen)</option>
                <option value="stretch">Oprekken (SL naar minimum, TP volgt RR)</option>
              </select>
            </div>
          </div>

          <!-- MAX LOSS PER TRADE -->
          <div style="margin-top:12px;padding:10px;border:1px dashed rgba(248,113,113,.4);border-radius:6px;background:rgba(248,113,113,.05)">
            <label style="font-size:12px;font-weight:600;color:var(--red)">Max verlies per trade (€, 0=uit)</label>
            <input type="number" id="bt-max-loss-trade" value="0" min="0" max="500" step="5" style="width:100%;margin-top:6px;padding:6px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
            <div style="font-size:9px;color:var(--text3);margin-top:4px;line-height:1.4">
              Geld-gebaseerde lotsize: berekent lot per trade zodat verlies bij SL binnen deze grens blijft. Verre SL → kleinere lot. Overschrijft vaste lotsize.
            </div>
          </div>
        </div>
        ...
        <div style="margin-top:10px;padding:8px 10px;border:1px dashed rgba(251,191,36,.35);border-radius:6px;background:rgba(251,191,36,.04)">
          <label style="font-size:11px;color:var(--amber);font-weight:600;display:block;margin-bottom:4px">Optimizer minimum trades (in-sample)</label>
          <input type="number" id="opt-min-trades" value="200" min="20" max="2000" step="10" style="width:100%;padding:6px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
          <div style="font-size:9px;color:var(--text3);margin-top:4px;line-height:1.4">
            Combinaties met minder trades worden gediskwalificeerd vóór ranking. Voorkomt statistisch dunne "winnaars". OOS-drempel = 30% hiervan.
          </div>
        </div>
...
// ── Pair Chip toggle (multi-pair selection) ──
function togglePairChip(el){
  ...
  const active = [...container.querySelectorAll(".pair-chip.active")].map(p => p.dataset.pair);
  if(active.length === 0){ el.click(); return; }  // minimum 1 pair
  // Sync hidden legacy input (first active pair, or BOTH-style combo)
  const hidden = container.parentElement.querySelector('input[type="hidden"]');
  if(hidden) hidden.value = active[0];
}

function getSelectedPairs(chipContainerId){
  const cont = document.getElementById(chipContainerId);
  if(!cont) return null;
  const active = [...cont.querySelectorAll(".pair-chip.active")].map(p => p.dataset.pair);
  return active.length ? active : ["EURUSD"];
}

function setSelectedPairs(chipContainerId, pairs){
  const cont = document.getElementById(chipContainerId);
  if(!cont || !pairs || !pairs.length) return;
  cont.querySelectorAll(".pair-chip").forEach(chip => {
    const on = pairs.includes(chip.dataset.pair);
    chip.classList.toggle("active", on);
    chip.style.background = on ? "rgba(255,255,255,.15)" : "rgba(255,255,255,.03)";
    chip.style.color = on ? "var(--glow2)" : "var(--text2)";
    chip.style.borderColor = on ? "var(--glow2)" : "var(--border2)";
...
// ── MT5 Health Check (full round-trip diagnostic) ──
async function runMT5HealthCheck(){
  const btn = document.getElementById("mt5-health-btn");
  const resultEl = document.getElementById("mt5-health-result");
  const originalTxt = btn.innerHTML;
  btn.disabled = true;
  btn.innerHTML = "⏳ Testing MT5 bridge...";
  resultEl.style.display = "none";
  try{
    const r = await fetch("/api/mt5/health", {method:"POST"});
    const d = await r.json();
    let html = `<div style="padding:8px;border-radius:5px;background:${d.ok ? "rgba(52,211,153,.12)" : "rgba(248,113,113,.12)"};border:1px solid ${d.ok ? "rgba(52,211,153,.4)" : "rgba(248,113,113,.4)"};margin-bottom:6px">`;
    html += `<div style="font-weight:600;color:${d.ok ? "var(--green)" : "var(--red)"}">${d.ok ? "✅" : "⚠️"} ${d.summary}</div></div>`;
    for(const chk of d.checks){
      const color = chk.ok ? "var(--green)" : "var(--red)";
      const icon = chk.ok ? "✓" : "✗";
      html += `<div style="padding:5px 8px;border-left:2px solid ${color};margin-bottom:3px;background:rgba(0,0,0,.2)">
        <div style="color:${color};font-weight:600">${icon} ${chk.step}</div>
        <div style="color:var(--text2);font-size:9px;margin-top:2px">${chk.detail}</div>
      </div>`;
    }
    resultEl.innerHTML = html;
    resultEl.style.display = "block";
  }catch(e){
    resultEl.innerHTML = `<div style="color:var(--red);padding:8px">Error: ${e.message}</div>`;
    resultEl.style.display = "block";
  }finally{
    btn.disabled = false;
...
// ── Test Trade (real 0.01 lot, auto-close in 10s) ──
async function runMT5TestTrade(){
  const symbol = document.getElementById("mt5-test-symbol").value;
  const side = document.getElementById("mt5-test-side").value;
  if(!confirm(`Send REAL ${side} ${symbol} 0.01 lot order to MT5?\n\nWill be automatically closed after 10 seconds.\nExpected cost: €0.20-0.50 (spread + slippage).`)) return;
  const btn = document.getElementById("mt5-test-btn");
  const resultEl = document.getElementById("mt5-test-result");
  const originalTxt = btn.innerHTML;
  btn.disabled = true;
  btn.innerHTML = "⏳ Placing order... (~15s total)";
  resultEl.style.display = "none";
  try{
    const r = await fetch("/api/mt5/test_trade", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({symbol, side, hold_seconds: 10}),
    });
    const d = await r.json();
    let html = `<div style="padding:8px;border-radius:5px;background:${d.ok ? "rgba(52,211,153,.12)" : "rgba(248,113,113,.12)"};border:1px solid ${d.ok ? "rgba(52,211,153,.4)" : "rgba(248,113,113,.4)"};margin-bottom:6px">`;
    html += `<div style="font-weight:600;color:${d.ok ? "var(--green)" : "var(--red)"}">${d.ok ? "✅ TEST PASSED" : "❌ TEST FAILED"}</div>`;
    if(d.result && d.result.summary) html += `<div style="color:var(--text2);font-size:9px;margin-top:4px">${d.result.summary}</div>`;
    if(d.error) html += `<div style="color:var(--red);font-size:9px;margin-top:4px">${d.error}</div>`;
    if(d.manual_close_ticket) html += `<div style="color:var(--amber);font-weight:600;margin-top:4px">⚠ MANUAL CLOSE REQUIRED: Ticket #${d.manual_close_ticket}</div>`;
    html += `</div>`;
    if(d.result && d.result.phases){
      for(const p of d.result.phases){
        const color = p.ok ? "var(--green)" : "var(--red)";
        const icon = p.ok ? "✓" : "✗";
        html += `<div style="padding:5px 8px;border-left:2px solid ${color};margin-bottom:3px;background:rgba(0,0,0,.2)">
          <div style="color:${color};font-weight:600">${icon} ${p.phase}</div>
          <div style="color:var(--text2);font-size:9px;margin-top:2px">${p.detail}</div>
        </div>`;
      }
    }
    resultEl.innerHTML = html;
...
// ── Mobile MT5 Health Check ──
async function runMT5HealthCheckMobile(){
  const btn = document.getElementById("m-mt5-health-btn");
  const resultEl = document.getElementById("m-mt5-health-result");
  ...
  try{
    ...
    const d = await r.json();
    let html = `<div style="padding:8px;border-radius:5px;background:${d.ok ? "rgba(52,211,153,.12)" : "rgba(248,113,113,.12)"};border:1px solid ${d.ok ? "rgba(52,211,153,.4)" : "rgba(248,113,113,.4)"};margin-bottom:6px">`;
    html += `<div style="font-weight:600;color:${d.ok ? "#34d399" : "#f87171"}">${d.ok ? "✅" : "⚠️"} ${d.summary}</div></div>`;
    for(const chk of d.checks){
      const color = chk.ok ? "#34d399" : "#f87171";
      const icon = chk.ok ? "✓" : "✗";
      html += `<div style="padding:5px 8px;border-left:2px solid ${color};margin-bottom:3px;background:rgba(0,0,0,.2)">
        <div style="color:${color};font-weight:600">${icon} ${chk.step}</div>
        <div style="color:var(--text2);font-size:9px;margin-top:2px">${chk.detail}</div>
    ...
    resultEl.innerHTML = `<div style="color:#f87171;padding:8px">Error: ${e.message}</div>`;
    resultEl.style.display = "block";
  }finally{
    btn.disabled = false;
    btn.innerHTML = originalTxt;
...
// ── Mobile Test Trade ──
async function runMT5TestTradeMobile(){
  const symbol = document.getElementById("m-mt5-test-symbol").value;
  const side = document.getElementById("m-mt5-test-side").value;
  if(!confirm(`Send REAL ${side} ${symbol} 0.01 lot order?\n\nAuto-closed after 10s.\nExpected cost: €0.20-0.50.`)) return;
  const btn = document.getElementById("m-mt5-test-btn");
  const resultEl = document.getElementById("m-mt5-test-result");
  if(!btn || !resultEl) return;
  const originalTxt = btn.innerHTML;
  btn.disabled = true;
  btn.innerHTML = "⏳ Placing order... (~15s)";
  resultEl.style.display = "none";
  try{
    const r = await fetch("/api/mt5/test_trade", {
      method:"POST", headers:{"Content-Type":"application/json"},
    ...
    const d = await r.json();
    let html = `<div style="padding:8px;border-radius:5px;background:${d.ok ? "rgba(52,211,153,.12)" : "rgba(248,113,113,.12)"};border:1px solid ${d.ok ? "rgba(52,211,153,.4)" : "rgba(248,113,113,.4)"};margin-bottom:6px">`;
    html += `<div style="font-weight:600;color:${d.ok ? "#34d399" : "#f87171"}">${d.ok ? "✅ TEST PASSED" : "❌ TEST FAILED"}</div>`;
    if(d.result && d.result.summary) html += `<div style="color:var(--text2);font-size:9px;margin-top:4px">${d.result.summary}</div>`;
    if(d.error) html += `<div style="color:#f87171;font-size:9px;margin-top:4px">${d.error}</div>`;
    if(d.manual_close_ticket) html += `<div style="color:#fbbf24;font-weight:600;margin-top:4px">⚠ MANUAL CLOSE REQUIRED: Ticket #${d.manual_close_ticket}</div>`;
    html += `</div>`;
    if(d.result && d.result.phases){
      for(const p of d.result.phases){
        const color = p.ok ? "#34d399" : "#f87171";
        const icon = p.ok ? "✓" : "✗";
        html += `<div style="padding:5px 8px;border-left:2px solid ${color};margin-bottom:3px;background:rgba(0,0,0,.2)">
          <div style="color:${color};font-weight:600">${icon} ${p.phase}</div>
          <div style="color:var(--text2);font-size:9px;margin-top:2px">${p.detail}</div>
...
async function runOptimizer(pairChoice){
  ...
  // Determine selected pairs + whether to use separate FX/XAU spread inputs
  const selectedPairs = getSelectedPairs("bt-pair-chips");
  const hasXau = selectedPairs.includes("XAUUSD");
  const hasFx  = selectedPairs.some(p => p === "EURUSD" || p === "GBPUSD");
  const isBoth = hasXau && hasFx;  // mixed → use bt-spread-eur/bt-spread-xau split fields
  const body = {
    ...
    lotsize_gbp:   parseFloat((document.getElementById("bt-lot-gbp")||{}).value)||parseFloat(document.getElementById("bt-lot-eur").value)||1,
    ...
    min_trades:    parseInt(document.getElementById("opt-min-trades").value)||200,
  };
...
function renderOptimizer(d){
  // Meta info
  const dq = d.disqualified != null ? ` · ${d.disqualified} gediskwalificeerd (< ${d.min_trades_is} trades)` : "";
  document.getElementById("opt-meta").textContent =
    `${d.total_tested} combinaties getest · ${d.passed_threshold||d.results.length} door drempel${dq} · Split: ${d.split_date}`;
  ...
  document.getElementById("opt-summary").innerHTML = `
    ...
    <div class="bt-stat"><div class="l">Min trades drempel</div><div class="v" style="color:var(--amber)">${d.min_trades_is||"—"}</div></div>
  ...
  if(!d.results || !d.results.length){
    tbody.innerHTML = `<tr><td colspan="15" style="text-align:center;padding:24px;color:var(--text3)">Geen enkele combinatie haalde de drempel van ${d.min_trades_is} trades. Verlaag de drempel of verbreed de periode.</td></tr>`;
    return;
  }
  ...
  d.results.forEach((r,i)=>{
    ...
    const oos_thin = r.oos_sufficient === false;  // OOS below reliability threshold
    const row_bg   = oos_thin ? "rgba(248,113,113,.06)" :
                     (oos_good && is_good ? "rgba(52,211,153,.04)" :
                     oos_good ? "rgba(251,191,36,.04)" : "");
    const tr = document.createElement("tr");
    tr.style.background = row_bg;
    tr.dataset.config = JSON.stringify(r);
    const oosTradeCell = oos_thin
      ? `<td style="color:var(--red)" title="Te weinig OOS trades — onbetrouwbaar">${r.oos_trades} ⚠</td>`
...
function applyConfig(r){
  // Load backtest config from saved run
  document.getElementById("bt-pair").value  = r.pair;
  // Restore chip selection from pairs array (fallback to single pair or BOTH→EUR+XAU)
  if(r.pairs && r.pairs.length){
    setSelectedPairs("bt-pair-chips", r.pairs);
  } else if(r.pair === "BOTH"){
    setSelectedPairs("bt-pair-chips", ["EURUSD","XAUUSD"]);
  } else if(r.pair){
    setSelectedPairs("bt-pair-chips", [r.pair]);
  }
  updateBtPairFields();
...
async function runBacktest(){
  ...
  const body={
    ...
    limit_mode:document.getElementById("bt-limit-mode").checked,
    limit_expiry_bars:parseInt(document.getElementById("bt-limit-expiry").value)||3,
    fvg_select:document.getElementById("bt-fvg-select").value,
    sl_mode:document.getElementById("bt-sl-mode").value,
    atr_mult:parseFloat(document.getElementById("bt-atr-mult").value)||1.5,
    max_sl_pips:parseFloat(document.getElementById("bt-max-sl").value)||0,
    min_sl_pips:parseFloat(document.getElementById("bt-min-sl").value)||0,
    min_sl_mode:document.getElementById("bt-min-sl-mode").value,
    max_loss_eur:parseFloat(document.getElementById("bt-max-loss-trade").value)||0,
...
function renderBtStats(s,trades){
  ...
  // Limit-order fill timing diagnostics
  const limitBox = document.getElementById("bt-limit-diag");
  if(s.limit_fill_timing){
    const timing = s.limit_fill_timing;
    let timingHtml = Object.entries(timing).map(([bars,count]) =>
      `<span style="display:inline-block;margin-right:12px">candle ${bars}: <b style="color:var(--glow2)">${count}</b></span>`).join("");
    if(limitBox){
      limitBox.style.display = "block";
      limitBox.innerHTML = `
        <div style="font-size:11px;font-weight:600;color:var(--glow2);margin-bottom:6px">📊 Limit Order Fill Timing</div>
        <div style="font-size:10px;color:var(--text2);line-height:1.6">
          <div>Gevulde orders: <b>${s.limit_filled_count}</b> · Gem. fill na <b>${s.limit_avg_fill_bars}</b> candles</div>
          <div style="margin-top:4px">${timingHtml}</div>
          <div style="margin-top:6px;color:var(--text3);font-size:9px">Vergelijk deze totalen met een normale (market-mode) run om te zien hoeveel trades je afvalt en wat het met WR/P&L doet.</div>
        </div>`;
    }
  } else if(limitBox){
...
function updateBtPairFields(){
  // Chip mode: use pairs array to determine mixed-pair display
  const selectedPairs = getSelectedPairs("bt-pair-chips");
  const hasXau = selectedPairs.includes("XAUUSD");
  const hasFx  = selectedPairs.some(p => p === "EURUSD" || p === "GBPUSD");
  // Mixed FX+XAU needs separate spread fields (different pip cost)
  const mixed = hasXau && hasFx;
  const spreadSingle = document.getElementById("bt-spread-single");
  const spreadBoth   = document.getElementById("bt-spread-both");
  if(spreadSingle) spreadSingle.style.display = mixed ? "none" : "";
  if(spreadBoth)   spreadBoth.style.display   = mixed ? "" : "none";
  // Default spread depending on primary pair type
  if(!mixed){
    if(hasXau && !hasFx){
  ...
  // Sync hidden pair input (first selected, for backward compat)
  if(selectedPairs.length > 0){
    document.getElementById("bt-pair").value = selectedPairs[0];
  }
}
...
async function savePreset(){
  ...
  const cfg = {
    ...
    pairs:       getSelectedPairs("lt-pair-chips"),
    ...
    lotsize_gbp: parseFloat((document.getElementById("lt-lot-gbp")||{}).value)||parseFloat(document.getElementById("lt-lot-eur").value)||1,
    ...
    fvg_select:   document.getElementById("lt-fvg-select") ? document.getElementById("lt-fvg-select").value : "nearest",
    sl_mode:      document.getElementById("lt-sl-mode") ? document.getElementById("lt-sl-mode").value : "structural",
    atr_mult:     document.getElementById("lt-atr-mult") ? (parseFloat(document.getElementById("lt-atr-mult").value)||1.5) : 1.5,
    max_sl_pips:  document.getElementById("lt-max-sl") ? (parseFloat(document.getElementById("lt-max-sl").value)||0) : 0,
    min_sl_pips:  document.getElementById("lt-min-sl") ? (parseFloat(document.getElementById("lt-min-sl").value)||0) : 0,
    min_sl_mode:  document.getElementById("lt-min-sl-mode") ? document.getElementById("lt-min-sl-mode").value : "off",
    max_loss_eur: document.getElementById("lt-max-loss-trade") ? (parseFloat(document.getElementById("lt-max-loss-trade").value)||0) : 0,
    be_trigger:   document.getElementById("lt-be-trigger") ? (parseFloat(document.getElementById("lt-be-trigger").value)||0) : 0,
...
async function loadPresetIntoForm(name){
  ...
  if(c.pairs && c.pairs.length) setSelectedPairs("lt-pair-chips", c.pairs);
  else if(c.pair) setSelectedPairs("lt-pair-chips", c.pair === "BOTH" ? ["EURUSD","XAUUSD"] : [c.pair]);
  ...
  if(c.lotsize_gbp && document.getElementById("lt-lot-gbp")) document.getElementById("lt-lot-gbp").value = c.lotsize_gbp;
  ...
  // ATR-SL + sizing (new)
  if(document.getElementById("lt-fvg-select") && c.fvg_select) document.getElementById("lt-fvg-select").value = c.fvg_select;
  if(document.getElementById("lt-sl-mode") && c.sl_mode){
    document.getElementById("lt-sl-mode").value = c.sl_mode;
    document.getElementById("lt-atr-opts").style.display = c.sl_mode==="atr"?"block":"none";
  }
  if(document.getElementById("lt-atr-mult") && c.atr_mult != null) document.getElementById("lt-atr-mult").value = c.atr_mult;
  if(document.getElementById("lt-max-sl") && c.max_sl_pips != null) document.getElementById("lt-max-sl").value = c.max_sl_pips;
  if(document.getElementById("lt-min-sl") && c.min_sl_pips != null) document.getElementById("lt-min-sl").value = c.min_sl_pips;
  if(document.getElementById("lt-min-sl-mode") && c.min_sl_mode) document.getElementById("lt-min-sl-mode").value = c.min_sl_mode;
  if(document.getElementById("lt-max-loss-trade") && c.max_loss_eur != null) document.getElementById("lt-max-loss-trade").value = c.max_loss_eur;
  if(document.getElementById("lt-be-trigger") && c.be_trigger != null) document.getElementById("lt-be-trigger").value = c.be_trigger;
}
  ...
  if(!d.closed_trades || !d.closed_trades.length){
    ...
    ctb.innerHTML = [...d.closed_trades].reverse().map(t=>{
      ...
      return `<tr>
        ...
        <td>
          <button onclick="showTradeContext(${t.id})" style="font-size:9px;padding:1px 6px;border:1px solid rgba(255,255,255,.2);border-radius:3px;background:transparent;color:var(--glow2);cursor:pointer;margin-right:3px" title="View full context">🔍</button>
          <button onclick="deleteTrade(${t.id})" style="font-size:9px;padding:1px 6px;border:1px solid rgba(248,113,113,.3);border-radius:3px;background:transparent;color:var(--red);cursor:pointer">X</button>
        </td>
      </tr>`;
    }).join("");
  }
...
function exportLiveCSV(){
  // Rich export with full trade context (server-side, ~45 fields per trade)
...
async function showTradeContext(tradeId){
  try{
    const r = await fetch(`/api/trades/context/${tradeId}`);
    const d = await r.json();
    if(!d.ok){ alert("Trade context not found"); return; }
    const t = d.trade;
    const ctx = t.context || {};
    // Build grouped display
    const groups = {
      "Trade": {
        "Pair": t.pair, "Direction": t.direction, "Entry": t.entry_price,
        "Exit": t.exit_price||"—", "Pips": t.pips, "P&L €": (t.pnl_eur||0).toFixed(2),
        "Outcome": t.outcome_label||t.outcome, "R-multiple": t.r_multiple,
        "MAE pips": t.mae_pips, "MFE pips": t.mfe_pips, "Duration min": t.duration_min,
        "Exit reason": t.exit_reason,
      },
      "Time": {
        "Hour": ctx.ctx_hour, "Weekday": ctx.ctx_weekday, "Session": ctx.ctx_session,
        "Killzone": ctx.ctx_killzone, "Friday": ctx.ctx_is_friday, "Monday": ctx.ctx_is_monday,
      },
      "Volatility": {
        "ATR14 pips": ctx.ctx_atr14_pips, "ATR percentile": ctx.ctx_atr_percentile,
        "Avg body 10": ctx.ctx_avg_body_pips_10, "Current body": ctx.ctx_current_body_pips,
        "Largest range 20": ctx.ctx_largest_range_pips_20,
        "Day range": ctx.ctx_day_range_pips, "Week range": ctx.ctx_week_range_pips,
      },
      "Structure": {
        "Trend current": ctx.ctx_trend_current, "HTF bias": ctx.ctx_htf_bias,
        "Pos in day range %": ctx.ctx_pos_in_day_range, "Pos in week range %": ctx.ctx_pos_in_week_range,
        "Dist to day high": ctx.ctx_dist_to_day_high_pips, "Dist to day low": ctx.ctx_dist_to_day_low_pips,
      },
      "FVG Setup": {
        "FVG size pips": ctx.ctx_fvg_size_pips, "FVG age bars": ctx.ctx_fvg_age_bars,
        "Entry vs FVG mid": ctx.ctx_entry_vs_fvg_mid_pips, "FVG vs EQ": ctx.ctx_fvg_vs_equilibrium,
      },
      "Judges": {
        "Score": ctx.ctx_bias_score, "Aligned": ctx.ctx_judges_aligned,
        "J1": ctx.ctx_j1, "J2": ctx.ctx_j2, "J3": ctx.ctx_j3, "J4": ctx.ctx_j4, "J5": ctx.ctx_j5,
      },
      "Context": {
        "Open trades": ctx.ctx_open_trades_count, "Trades today": ctx.ctx_trades_today,
        "Daily P&L so far": ctx.ctx_daily_pnl_so_far, "Last result": ctx.ctx_last_trade_result,
      },
      "News & Macro": {
        "High-impact news today": ctx.ctx_high_impact_news_today,
        "High-impact within 60min": ctx.ctx_high_impact_within_60min,
        "Spread at entry": ctx.ctx_spread_at_entry_pips,
        "DXY value": ctx.ctx_dxy_value,
        "DXY change %": ctx.ctx_dxy_change_pct,
        "DXY direction": ctx.ctx_dxy_direction,
      },
      "Price Levels & Momentum": {
        "Dist to round (50p)": ctx.ctx_dist_to_round_pips,
        "Dist to big figure": ctx.ctx_dist_to_big_figure_pips,
        "Momentum streak": ctx.ctx_momentum_streak,
        "Streak direction": ctx.ctx_momentum_direction,
        "Streak aligns trade": ctx.ctx_streak_aligns_trade,
      },
    };
    let html = `<div style="font-size:14px;font-weight:600;margin-bottom:12px;color:var(--glow2)">Trade #${t.id} — Full Context</div>`;
    for(const [gname, fields] of Object.entries(groups)){
      html += `<div style="margin-bottom:10px"><div style="font-size:10px;text-transform:uppercase;color:var(--text3);letter-spacing:1px;margin-bottom:4px;border-bottom:1px solid rgba(255,255,255,.1);padding-bottom:2px">${gname}</div>`;
      html += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:2px 12px;font-size:11px">`;
      for(const [k, v] of Object.entries(fields)){
        if(v === undefined || v === null) continue;
        html += `<div style="color:var(--text3)">${k}</div><div style="color:var(--text);text-align:right">${v}</div>`;
      }
      html += `</div></div>`;
    }
    // Modal overlay
    let modal = document.getElementById("trade-ctx-modal");
    if(!modal){
      modal = document.createElement("div");
      modal.id = "trade-ctx-modal";
      modal.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px";
      modal.onclick = (e)=>{ if(e.target===modal) modal.remove(); };
      document.body.appendChild(modal);
    }
    modal.innerHTML = `<div style="background:var(--bg2);border:1px solid var(--border2);border-radius:10px;padding:20px;max-width:600px;max-height:80vh;overflow-y:auto;width:100%">
      ${html}
      <button onclick="document.getElementById('trade-ctx-modal').remove()" style="margin-top:12px;width:100%;padding:8px;border:1px solid var(--border2);border-radius:5px;background:rgba(255,255,255,.05);color:var(--text);cursor:pointer">Close</button>
    </div>`;
  }catch(e){
    alert("Error loading context: " + e.message);
  }
}
...
<div class="page active" id="page-live">
  ...
  <div class="card">
    ...
    <div class="card-body collapsed" id="m-cfg-body">
      <div class="form-row">
        <div class="form-group">
          <label>Pairs (select 1 or more)</label>
          <div id="m-pair-chips" style="display:flex;gap:6px;flex-wrap:wrap;margin-top:4px">
            <div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--glow2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.15);color:var(--glow2);user-select:none;transition:.15s;font-weight:600">EURUSD</div>
            <div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">GBPUSD</div>
            <div class="pair-chip" data-pair="XAUUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">XAUUSD</div>
          </div>
          <input type="hidden" id="m-pair" value="EURUSD">
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          <label>Lot GBP (micro)</label>
          <input type="number" id="m-lot-gbp" value="1" min="1" inputmode="numeric">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
      ...
      <div class="form-group" style="margin-top:10px">
        <label>FVG selectie</label>
        <select id="m-fvg-select" style="width:100%;padding:8px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
          <option value="nearest">Dichtstbijzijnde (huidig)</option>
          <option value="first">Eerste van cluster</option>
        </select>
      </div>
      <div class="form-group" style="margin-top:10px">
        <label>SL berekening</label>
        <select id="m-sl-mode" onchange="document.getElementById('m-atr-opts').style.display=this.value==='atr'?'block':'none'" style="width:100%;padding:8px;font-size:12px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
          <option value="structural">Structureel (swing — huidig)</option>
          <option value="atr">ATR-gebaseerd</option>
        </select>
      </div>
      <div id="m-atr-opts" style="display:none">
        <div class="form-group"><label>ATR multiplier</label><input type="number" id="m-atr-mult" value="1.5" min="0.5" max="5" step="0.1" inputmode="decimal"></div>
      </div>
      <div class="form-group"><label>Max SL afstand (pips, 0=uit)</label><input type="number" id="m-max-sl" value="0" min="0" max="200" step="5" inputmode="decimal"></div>
      <div class="form-group"><label>Min SL afstand (pips, 0=uit)</label><input type="number" id="m-min-sl" value="0" min="0" max="100" step="1" inputmode="decimal"></div>
      <div class="form-group"><label>Bij te krappe SL</label>
        <select id="m-min-sl-mode" style="width:100%;padding:8px;font-size:13px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:6px">
          <option value="off">Uit</option>
          <option value="skip">Overslaan</option>
          <option value="stretch">Oprekken (TP volgt RR)</option>
        </select>
      </div>
      <div class="form-group"><label>Max verlies per trade (€, 0=uit)</label><input type="number" id="m-max-loss-trade" value="0" min="0" max="500" step="5" inputmode="decimal"></div>
      <div class="form-group"><label>Break-even trigger (R, 0=uit)</label><input type="number" id="m-be-trigger" value="0" min="0" max="5" step="0.1" inputmode="decimal"></div>
      <div style="font-size:9px;color:var(--amber);line-height:1.4;padding:6px 8px;background:rgba(251,191,36,.06);border-radius:6px;margin-top:4px">
        ⚠ Max verlies per trade laat lotsize variëren. Test eerst in backtester.
      </div>
      ...
      <button onclick="runMT5HealthCheckMobile()" id="m-mt5-health-btn" style="margin-top:8px;width:100%;padding:10px;border:1px solid var(--border2);border-radius:5px;background:rgba(255,255,255,.05);color:var(--text);font-size:12px;font-family:'Inter',sans-serif;cursor:pointer">
        🔍 Run Full MT5 Health Check
      </button>
      <div id="m-mt5-health-result" style="margin-top:8px;display:none;font-size:10px;font-family:'JetBrains Mono',monospace;line-height:1.5"></div>

      <!-- Live test trade -->
      <div style="margin-top:10px;padding:10px;border:1px dashed rgba(251,191,36,.4);border-radius:5px;background:rgba(251,191,36,.05)">
        <div style="font-size:11px;color:var(--amber);font-weight:600;margin-bottom:4px">⚠ Live Test Trade (REAL order)</div>
        <div style="font-size:10px;color:var(--text3);line-height:1.4;margin-bottom:8px">
          Real 0.01 lot → auto-close in 10s. Cost: ~€0.20-0.50.
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
          <select id="m-mt5-test-symbol" style="padding:8px;font-size:11px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
            <option value="EURUSD">EURUSD</option>
            <option value="GBPUSD">GBPUSD</option>
            <option value="XAUUSD">XAUUSD</option>
          </select>
          <select id="m-mt5-test-side" style="padding:8px;font-size:11px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:4px">
            <option value="BUY">BUY</option>
            <option value="SELL">SELL</option>
          </select>
        </div>
        <button onclick="runMT5TestTradeMobile()" id="m-mt5-test-btn" style="width:100%;padding:10px;border:1px solid var(--amber);border-radius:5px;background:rgba(251,191,36,.15);color:var(--amber);font-size:12px;font-weight:600;cursor:pointer">
          📤 Send Test Trade (0.01 lot)
        </button>
        <div id="m-mt5-test-result" style="margin-top:8px;display:none;font-size:10px;font-family:'JetBrains Mono',monospace;line-height:1.5"></div>
      </div>
    </div>
  </div>
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="form-row">
        <div class="form-group">
          <label>Pairs (select 1 or more)</label>
          <div id="bt-m-pair-chips" style="display:flex;gap:6px;flex-wrap:wrap;margin-top:4px">
            <div class="pair-chip active" data-pair="EURUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--glow2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.15);color:var(--glow2);user-select:none;transition:.15s;font-weight:600">EURUSD</div>
            <div class="pair-chip" data-pair="GBPUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">GBPUSD</div>
            <div class="pair-chip" data-pair="XAUUSD" onclick="togglePairChip(this)" style="padding:6px 12px;border:1px solid var(--border2);border-radius:16px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.03);color:var(--text2);user-select:none;transition:.15s">XAUUSD</div>
          </div>
          <input type="hidden" id="bt-m-pair" value="EURUSD">
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          <label>Lot GBP (micro)</label>
          <input type="number" id="bt-m-lot-gbp" value="1" min="1" inputmode="numeric">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
      ...
      <div class="toggle-section-title">SL & FVG (ICT)</div>
      <div class="form-group">
        <label>FVG selectie</label>
        <select id="bt-m-fvg-select" style="width:100%;padding:8px;font-size:13px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:6px">
          <option value="nearest">Dichtstbijzijnde (huidig)</option>
          <option value="first">Eerste van cluster</option>
        </select>
      </div>
      <div class="form-group">
        <label>SL berekening</label>
        <select id="bt-m-sl-mode" onchange="document.getElementById('bt-m-atr-opts').style.display=this.value==='atr'?'block':'none'" style="width:100%;padding:8px;font-size:13px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:6px">
          <option value="structural">Structureel (swing)</option>
          <option value="atr">ATR-gebaseerd</option>
        </select>
      </div>
      <div id="bt-m-atr-opts" style="display:none">
        <div class="form-group"><label>ATR multiplier</label><input type="number" id="bt-m-atr-mult" value="1.5" min="0.5" max="5" step="0.1" inputmode="decimal"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label>Max SL (pips, 0=uit)</label><input type="number" id="bt-m-max-sl" value="0" min="0" max="200" step="5" inputmode="decimal"></div>
        <div class="form-group"><label>Max verlies/trade (€)</label><input type="number" id="bt-m-max-loss-trade" value="0" min="0" max="500" step="5" inputmode="decimal"></div>
      </div>
      <div class="form-group"><label>Min SL (pips, 0=uit)</label><input type="number" id="bt-m-min-sl" value="0" min="0" max="100" step="1" inputmode="decimal"></div>
      <div class="form-group">
        <label>Bij te krappe SL</label>
        <select id="bt-m-min-sl-mode" style="width:100%;padding:8px;font-size:13px;background:rgba(0,0,0,.4);color:var(--text);border:1px solid var(--border2);border-radius:6px">
          <option value="off">Uit</option>
          <option value="skip">Overslaan</option>
          <option value="stretch">Oprekken (TP volgt RR)</option>
        </select>
...
// ── Chip helpers (pairs + timeframes) ──
function togglePairChip(el){
  const container = el.parentElement;
  el.classList.toggle("active");
  const on = el.classList.contains("active");
  el.style.background = on ? "rgba(255,255,255,.15)" : "rgba(255,255,255,.03)";
  el.style.color = on ? "var(--glow2)" : "var(--text2)";
  el.style.borderColor = on ? "var(--glow2)" : "var(--border2)";
  el.style.fontWeight = on ? "600" : "400";
  const active = [...container.querySelectorAll(".pair-chip.active")].map(p => p.dataset.pair);
  if(active.length === 0){ el.click(); return; }
  const hidden = container.parentElement.querySelector('input[type="hidden"]');
  if(hidden) hidden.value = active[0];
}
...
function toggleTfChip(el){
  ...
  el.style.borderColor = on ? "var(--glow2)" : "var(--border2)";
  el.style.fontWeight = on ? "600" : "400";
  const active = [...container.querySelectorAll(".tf-chip.active")].map(t => t.dataset.tf);
  if(active.length === 0){ el.click(); return; }
  const hidden = document.getElementById("bt-m-tf");
  if(hidden){
    let tfStr;
    if(active.length === 1) tfStr = active[0];
    else if(active.length === 3) tfStr = "ALL";
...
function mBuildConfig(){
  return {
    ...
    pairs:      getSelectedPairs("m-pair-chips"),
    ...
    lotsize_gbp:parseFloat((document.getElementById("m-lot-gbp")||{}).value)||parseFloat(document.getElementById("m-lot-eur").value)||1,
    ...
    fvg_select:   document.getElementById("m-fvg-select") ? document.getElementById("m-fvg-select").value : "nearest",
    sl_mode:      document.getElementById("m-sl-mode") ? document.getElementById("m-sl-mode").value : "structural",
    atr_mult:     document.getElementById("m-atr-mult") ? (parseFloat(document.getElementById("m-atr-mult").value)||1.5) : 1.5,
    max_sl_pips:  document.getElementById("m-max-sl") ? (parseFloat(document.getElementById("m-max-sl").value)||0) : 0,
    min_sl_pips:  document.getElementById("m-min-sl") ? (parseFloat(document.getElementById("m-min-sl").value)||0) : 0,
    min_sl_mode:  document.getElementById("m-min-sl-mode") ? document.getElementById("m-min-sl-mode").value : "off",
    max_loss_eur: document.getElementById("m-max-loss-trade") ? (parseFloat(document.getElementById("m-max-loss-trade").value)||0) : 0,
    be_trigger:   document.getElementById("m-be-trigger") ? (parseFloat(document.getElementById("m-be-trigger").value)||0) : 0,
...
// Show/hide XAU spread/slip row when chip selection has both XAU + FX
function _updateBtMXauRow(){
  const el = document.getElementById("bt-m-xau-row");
  if(!el) return;
  const pairs = getSelectedPairs("bt-m-pair-chips");
  const hasXau = pairs.includes("XAUUSD");
  const hasFx  = pairs.some(p => p === "EURUSD" || p === "GBPUSD");
  el.style.display = (hasXau && hasFx) ? "" : "none";
}
// Trigger on chip clicks (via a MutationObserver on the chip container)
setTimeout(() => {
  const cont = document.getElementById("bt-m-pair-chips");
  if(cont){
    cont.addEventListener("click", () => setTimeout(_updateBtMXauRow, 20));
    _updateBtMXauRow();
  }
}, 500);
...
async function mRunBacktest(){
  ...
  const selectedPairs = getSelectedPairs("bt-m-pair-chips");
  const hasXau = selectedPairs.includes("XAUUSD");
  const hasFx  = selectedPairs.some(p => p === "EURUSD" || p === "GBPUSD");
  const isBoth = hasXau && hasFx;
  const body = {
    ...
    lotsize_gbp: parseFloat((document.getElementById("bt-m-lot-gbp")||{}).value)||parseFloat(document.getElementById("bt-m-lot-eur").value)||1,
    ...
    fvg_select:  document.getElementById("bt-m-fvg-select") ? document.getElementById("bt-m-fvg-select").value : "nearest",
    sl_mode:     document.getElementById("bt-m-sl-mode") ? document.getElementById("bt-m-sl-mode").value : "structural",
    atr_mult:    parseFloat(document.getElementById("bt-m-atr-mult") ? document.getElementById("bt-m-atr-mult").value : 1.5)||1.5,
    max_sl_pips: parseFloat(document.getElementById("bt-m-max-sl") ? document.getElementById("bt-m-max-sl").value : 0)||0,
    min_sl_pips: parseFloat(document.getElementById("bt-m-min-sl") ? document.getElementById("bt-m-min-sl").value : 0)||0,
    min_sl_mode: document.getElementById("bt-m-min-sl-mode") ? document.getElementById("bt-m-min-sl-mode").value : "off",
    max_loss_eur: parseFloat(document.getElementById("bt-m-max-loss-trade") ? document.getElementById("bt-m-max-loss-trade").value : 0)||0,
  };
  try {
    const r = await fetch("/api/backtest", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(body)});
    const d = await r.json();
