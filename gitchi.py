    ...
    try:
        ...
        payload = {
            "embeds": [{
                "description": message,
                "color": color,
                "footer": {"text": f"GAMAN Trading · {datetime.datetime.now().strftime('%H:%M:%S')}"}
            }]
        }
...
class LiveEngine:
    ...
    def start(self, config):
        ...
        self.log("START", f"Engine gestart — {pairs} {config.get('tf')} | RR:{config.get('rr')} | Score≥{config.get('min_score')}")
        self._discord(
            f"🚀 **GITCHI Engine Gestart**\n"
            f"Pair: **{pairs}** | TF: **{config.get('tf')}**\n"
            f"RR: {config.get('rr')} | Min Score: {config.get('min_score')}\n"
    ...
    def stop(self):
        ...
        self._discord("⛔ **GITCHI Engine Gestopt** door gebruiker", 0xf59e0b)
        self._save_state()

    def _is_weekend(self):
        now = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
        wd  = now.weekday()  # 0=Mon, 4=Fri, 5=Sat, 6=Sun
        # Market closed: Friday 22:00 UTC → Sunday 21:00 UTC (Brussels CEST)
        if wd == 5: return True                           # All Saturday
        if wd == 4 and now.hour >= 22: return True        # Friday after 22:00 UTC
        if wd == 6 and now.hour < 21:  return True        # Sunday before 21:00 UTC
        return False
    ...
    def _run(self):
        while self.running:
            try:
                ...
                self.scan_count += 1
                self.last_scan   = datetime.datetime.now().strftime("%H:%M:%S")
                ...
                if max_trades > 0 and n_open >= max_trades:
                    pass  # Geen nieuwe trades openen
                else:
                    if self.config.get("trade_both"):
                        for pair in ["EURUSD", "XAUUSD"]:
                            self._scan(pair_override=pair)
                    else:
                        tf_map = {"15M+1H":["15M","1H"],"1H+4H":["1H","4H"],"ALL":["15M","1H","4H"]}
                        tfs = tf_map.get(self.config.get("tf","1H"), [self.config.get("tf","1H")])
    ...
    def _scan(self, pair_override=None, tf_override=None):
        ...
        # Lotsize per pair — EURUSD en XAUUSD kunnen verschillende lotsizes hebben
        if pair == "XAUUSD":
        ...
        # Max risico per trade: bereken lotsize op basis van % van kapitaal
        max_risk_pct = float(cfg.get("max_risk_pct", 0))
        capital      = float(cfg.get("capital", 10000))
        if max_risk_pct > 0 and capital > 0:
            # Typische SL = 20 pips voor EURUSD, 200 pips voor XAUUSD
            typical_sl = 20 if pair == "EURUSD" else 200
            max_risk_eur = capital * max_risk_pct / 100
            auto_lot = max_risk_eur / (typical_sl * pip_e)
            lotsize = max(1, round(auto_lot))
            self.log("INFO", f"Auto lotsize {pair}: {lotsize} (risico: €{max_risk_eur:.2f} = {max_risk_pct}%)")
        ...
        # Punt 2 — Consequent Encroachment: entry op 50% van de FVG (midpunt)
        # Dit is het exacte ICT niveau — niet de top of bottom maar het midden
        entry = (fvg["top"] + fvg["bottom"]) / 2

        # Punt 1 — Swing Low/High SL
        # Zoek de meest recente swing low (voor LONG) of swing high (voor SHORT)
        # in de laatste 20 bars VOOR de FVG formatie
        lookback_start = max(0, fvg["formed_at"] - 20)
        lookback_end   = fvg["formed_at"]

        if direction == "LONG":
            # Zoek swing low: bar waar low lager is dan buren
            swing_low = None
            for si in range(lookback_end-1, lookback_start, -1):
                if si < 1 or si >= n-1: continue
                l_curr = float(df.iloc[si]["low"])
                l_prev = float(df.iloc[si-1]["low"])
                l_next = float(df.iloc[si+1]["low"])
                if l_curr < l_prev and l_curr < l_next:
                    # Swing low gevonden — moet ONDER de FVG bottom liggen
                    if l_curr < fvg["bottom"]:
                        swing_low = l_curr
                        break
            if swing_low:
                sl = swing_low - pip_v * 2  # 2 pips onder swing low
            else:
                sl = fvg["bottom"] - pip_v * 15  # fallback: 15 pips onder FVG bottom

            risk = entry - sl
            if risk <= 0 or risk > entry * 0.05: return
            tp   = entry + risk * rr

        else:  # SHORT
            # Zoek swing high: bar waar high hoger is dan buren
            swing_high = None
            for si in range(lookback_end-1, lookback_start, -1):
                if si < 1 or si >= n-1: continue
                h_curr = float(df.iloc[si]["high"])
                h_prev = float(df.iloc[si-1]["high"])
                h_next = float(df.iloc[si+1]["high"])
                if h_curr > h_prev and h_curr > h_next:
                    # Swing high gevonden — moet BOVEN de FVG top liggen
                    if h_curr > fvg["top"]:
                        swing_high = h_curr
                        break
            if swing_high:
                sl = swing_high + pip_v * 2  # 2 pips boven swing high
            else:
                sl = fvg["top"] + pip_v * 15  # fallback: 15 pips boven FVG top
        ...
        trade = {
            ...
            "opened_ts":   int(datetime.datetime.now().timestamp()),
            "fvg_top":     fvg["top"],
        ...
        self.log("TRADE", f"▲ OPEN {direction} {pair} @ {entry:.5f} | SL:{sl:.5f} TP:{tp:.5f} | Score:{score} | {filters_str}")
        self._discord(
            f"**▲ TRADE GEOPEND — {pair}**\n"
            f"Richting: **{direction}** | TF: {tf}\n"
            f"Entry: `{entry:.5f}`\n"
            f"Stop Loss: `{sl:.5f}` ({sl_pips} pips)\n"
            f"Take Profit: `{tp:.5f}` ({round(sl_pips*rr,1)} pips)\n"
            f"Bias Score: **{score:+d}**\n"
    ...
    def _monitor_open_trades(self):
        ...
        for trade in trades_to_check:
            ...
            exit_price = None
            if direction=="LONG":
                if price <= sl:   hit="SL"; exit_price=sl
                elif price >= tp: hit="TP"; exit_price=tp
            else:
                if price >= sl:   hit="SL"; exit_price=sl
                elif price <= tp: hit="TP"; exit_price=tp
            ...
            if hit:
                ...
                closed = {
                    **trade,
                    ...
                    "closed_ts":   int(datetime.datetime.now().timestamp()),
                    "pips":        final_pips,
                    "pnl_eur":     final_pnl,
...
# yFinance hard limits: 15m = max 60 days, 1h = max 730 days
# For live chart we use short periods; for backtest we use start/end
...
def fetch_candles(pair, tf, start=None, end=None):
    ...
    try:
        ...
        if start and end:
            ...
            # yFinance hard limit: 15m only available for last 60 days from TODAY
            if iv == "15m":
                cutoff = now - datetime.timedelta(days=58)
                # Clamp start to cutoff if older
                effective_start = max(s, cutoff)
                if effective_start > e:
                    print(f"[DATA] 15M data niet beschikbaar voor {start} — ouder dan 60 dagen. Gebruik 1H voor langere periodes.")
                    # Fallback to 1h for older data
                    iv = "1h"
                    df = t.history(start=s, end=e, interval="1h")
                else:
                    # Fetch in 55-day chunks within the available window
                    chunks = []
                    ...
                    while chunk_start < e:
                        ...
                        try:
                            ...
                            print(f"[DATA] Chunk error {chunk_start}: {ce}")
                        chunk_start = chunk_end
                    if not chunks:
                        print(f"[DATA] Geen 15M data — fallback naar 1H")
                        iv = "1h"
                        df = t.history(start=s, end=e, interval="1h")
                    else:
                        df = pd.concat(chunks)
        ...
        if df is None or df.empty:
            print(f"[DATA] No data for {pair} {tf} {iv}")
            return None
        df = df[["Open","High","Low","Close"]].copy()
        df.columns = ["open","high","low","close"]
        if tf == "4H" or (iv == "1h" and tf == "4H"):
            df = df.resample("4h").agg({"open":"first","high":"max","low":"min","close":"last"}).dropna()
        print(f"[DATA] {pair} {tf} ({iv}) -> {len(df)} bars")
        return df
    except Exception as e:
        print(f"[DATA] Error {pair} {tf}: {e}")
        return None

def fetch_price(pair):
    """Haal live prijs op met meerdere fallback bronnen voor maximale accuraatheid."""
    sym = SYMBOLS.get(pair, "EURUSD=X")

    # Methode 1: yFinance fast_info (meest recent beschikbare)
    try:
        t = yf.Ticker(sym)
    ...
    # Methode 2: yFinance recent history (laatste candle close)
    try:
        t = yf.Ticker(sym)
...
def calc_bias(df, pair="EURUSD"):
    ...
    now_b=datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=2)))
    def _sess(h,m):
        t=h*60+m
...
def run_backtest(pair, tf, start, end, capital, lotsize, rr, use_ob, use_trend, use_eq, min_score, use_session=False, be_trigger=0.0, spread_pips=0.0, slippage_pips=0.0, lotsize_eur=None, lotsize_xau=None, max_daily_loss=0, max_trades=0, max_risk_pct=0, **kwargs):
...
<div id="topbar">
  ...
  <div class="topbar-right">
    ...
    <div id="refresh-countdown" style="font-size:10px;color:var(--text3);font-family:'JetBrains Mono',monospace;min-width:30px">30s</div>
...
<div id="app">
    ...
    <div class="card" id="live-open-card">
      ...
      <div class="card-body" style="padding:10px 16px">
        <div class="tbl-wrap" style="max-height:160px">
          <table>
            <thead><tr><th>#</th><th>Pair</th><th>Dir</th><th>Entry</th><th>Live Prijs</th><th>SL</th><th>TP</th><th>Live P&L</th><th>Filters</th><th>Geopend</th></tr></thead>
            <tbody id="live-open-tbody">
              <tr><td colspan="9" style="text-align:center;padding:16px;color:var(--text3)">Geen open posities</td></tr>
            </tbody>
          </table>
        </div>
      </div>
  ...
  <div id="right">
    ...
    <div class="card" id="live-engine-config">
      ...
      <div class="card-body">
        <div class="form-grid">
          ...
          <div class="form-group"><label>Risk:Reward ratio</label>
            <input type="number" id="lt-rr" value="2" min="0.5" step="0.5">
          </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px">
          <button class="btn-primary" id="lt-start-btn" onclick="startEngine()" style="margin:0;padding:10px">
            ▶ EXECUTE
          </button>
          <button id="lt-stop-btn" onclick="stopEngine()" disabled
            style="padding:10px;border:1px solid var(--red);border-radius:6px;background:rgba(248,113,113,.1);color:var(--red);font-family:'Inter',sans-serif;font-size:13px;font-weight:600;cursor:pointer;transition:.2s;opacity:.4">
            ■ SHUT DOWN
          </button>
        </div>
        <button onclick="closeAllTrades()" style="margin-top:8px;width:100%;padding:8px;border:1px solid rgba(248,113,113,.4);border-radius:6px;background:rgba(248,113,113,.06);color:var(--red);font-family:'Inter',sans-serif;font-size:12px;font-weight:600;cursor:pointer;transition:.2s;letter-spacing:.5px">
    ...
    <div class="card" id="bt-form" style="display:none">
      ...
      <div class="card-body">
        <div class="form-grid">
          <div class="form-group"><label>Pair</label><select id="bt-pair"><option value="EURUSD">EUR/USD</option><option value="XAUUSD">XAU/USD</option><option value="BOTH">BOTH</option></select></div>
        ...
        <button class="btn-primary" onclick="runBacktest()" id="bt-run">▶ Run Backtest</button>
        <button onclick="runOptimizer()" id="bt-opt"
          style="width:100%;margin-top:8px;padding:10px;border:1px solid var(--amber);border-radius:6px;background:rgba(251,191,36,.08);color:var(--amber);font-family:'Inter',sans-serif;font-size:13px;font-weight:600;cursor:pointer;transition:.2s;letter-spacing:.5px">
          ⚡ Optimaliseer — Beste Configuratie Vinden
        </button>
      </div>
    </div>
  </div>
...
async function loadCandles(){
  ...
  try{
    ...
    if(d.candles&&d.candles.length>0){
      ...
      document.getElementById("chart-status").textContent=`${d.candles.length} candles · ${S.pair} ${S.tf} · ⟳ 30s`;
    } else {
...
async function checkMarketStatus(){
  try{
    ...
    // Fallback: check client-side using browser time
    const now = new Date();
    const utcDay  = now.getUTCDay();   // 0=Sun, 5=Fri, 6=Sat
    const utcHour = now.getUTCHours();
    let isWeekend = false;
    if(utcDay === 6) isWeekend = true;                          // Saturday
    if(utcDay === 5 && utcHour >= 22) isWeekend = true;        // Friday after 22:00
    if(utcDay === 0 && utcHour < 21)  isWeekend = true;        // Sunday before 21:00 UTC (= 23:00 Brussels)
    const banner     = document.getElementById("weekend-banner");
    const openBanner = document.getElementById("market-open-banner");
...
function switchPage(page){
  S.page=page;
  document.querySelectorAll(".tab-btn").forEach((b,i)=>b.classList.toggle("active",(i===0&&page==="live")||(i===1&&page==="backtest")));
  ...
  document.getElementById("bt-form").style.display=live?"none":"";
...
async function runOptimizer(){
  ...
  const body = {
    ...
    spread_pips:   parseFloat(document.getElementById("bt-spread").value)||1.5,
    slippage_pips: parseFloat(document.getElementById("bt-slip").value)||0.5,
  };
...
async function runBacktest(){
  ...
  const body={
    ...
    spread_pips:parseFloat(document.getElementById("bt-spread").value)||0,
    slippage_pips:parseFloat(document.getElementById("bt-slip").value)||0,
...
async function startEngine(){
  const cfg = {
    ...
    rr:          document.getElementById("lt-rr").value,
    ...
    be_trigger:  0,  // BE verwijderd — SL wordt niet automatisch verplaatst
...
function renderEngineStatus(d){
  ...
  // Scan info
  document.getElementById("lt-scans").textContent = d.scan_count || 0;
  document.getElementById("lt-last-scan").textContent = d.last_scan ? "laatste: "+d.last_scan : "laatste: —";
  if(d.last_scan) document.getElementById("engine-scan-info").textContent = "laatste scan: "+d.last_scan;
  ...
  if(!d.open_trades || !d.open_trades.length){
    otb.innerHTML='<tr><td colspan="9" style="text-align:center;padding:16px;color:var(--text3)">Geen open posities</td></tr>';
  } else {
    otb.innerHTML = d.open_trades.map(t=>{
      const pnl = t.pnl_eur || 0;
      const pc  = pnl >= 0 ? "win" : "loss";
      return `<tr>
        <td>${t.id}</td><td>${t.pair}</td>
        <td><span class="pill ${t.direction==="LONG"?"pill-purple":"pill-amber"}">${t.direction}</span></td>
        <td>${t.entry_price}</td>
        ...
        <td style="font-size:9px;color:var(--text3)">${(t.opened_at||"").slice(11,16)}</td>
      </tr>`;
    }).join("");
...
<div style="position:relative;z-index:1">
  ...
  <div class="engine-status">
    ...
    <div style="text-align:right">
      <div class="engine-label">Laatste scan</div>
      <div class="engine-val" id="m-last-scan" style="font-size:11px;color:var(--text3)">—</div>
    </div>
  </div>
...
async function mPoll(){
  try{
    ...
    // Last scan
    document.getElementById("m-last-scan").textContent = ed.last_scan || "—";
    ...
    if(!ed.open_trades || !ed.open_trades.length){
      ...
      ot.innerHTML = ed.open_trades.map(t=>{
        ...
        return `<div class="trade-row">
          ...
          <div><div style="font-size:13px;font-weight:600">${t.pair}</div><div style="font-size:10px;color:var(--text3)">${t.entry_price} → ${t.live_price||"—"}</div></div>
          <div style="text-align:right"><div class="${pnl>=0?"win":"loss"}" style="font-size:14px">${pnl>=0?"+":""}€${pnl.toFixed(2)}</div><div style="font-size:9px;color:var(--text3)">${(t.opened_at||"").slice(11,16)}</div></div>
        </div>`;
      }).join("");
    }
