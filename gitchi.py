...
def run_backtest(pair, tf, start, end, capital, lotsize, rr, use_ob, use_trend, use_eq, min_score, use_session=False, use_sweep=False, use_htf_bias=False, use_smt=False, skip_asian=False, skip_holidays=False, require_htf_orderflow=False, require_dol=False, be_trigger=0.0, spread_pips=0.0, slippage_pips=0.0, lotsize_eur=None, lotsize_gbp=None, lotsize_xau=None, max_daily_loss=0, max_trades=0, max_risk_pct=0, **kwargs):
    ...
    for i in range(20, n - 2):
        ...
        # ── Punt 1 — SL berekening (met optionele ATR-mode + max-cap) ──
        # sl_mode: "structural" (swing low/high, huidig gedrag) of "atr" (volatiliteit-gebaseerd)
        ...
        atr_val_sl = calc_atr(df.iloc[:i+1], period=14) if i >= 15 else 0

        lb_sl_start = max(0, fvg["formed_at"] - 20)
        lb_sl_end   = fvg["formed_at"]

        if direction == "LONG":
            if bt_sl_mode == "atr" and atr_val_sl > 0:
                sl = raw_entry - atr_val_sl * bt_atr_mult
            else:
                swing_low = None
            ...
            # Max SL cap: skip trade if SL further than allowed
            if bt_max_sl_pips > 0 and (risk / pip_v) > bt_max_sl_pips: continue
        ...
        # ── Money-based position sizing (max loss per trade) ──
        # If max_loss_eur is set, compute lotsize so that a full-SL loss ≈ max_loss_eur.
        # This keeps euro-risk constant regardless of SL distance.
        eff_lotsize = lotsize
        max_loss_eur = float(kwargs.get("max_loss_eur", 0))
        if max_loss_eur > 0 and sl_pips > 0:
            # loss at SL = sl_pips * pip_e * lot → solve for lot given target loss
            eff_lotsize = round(max_loss_eur / (sl_pips * pip_e), 2)
            if eff_lotsize < 0.01: eff_lotsize = 0.01  # broker minimum
